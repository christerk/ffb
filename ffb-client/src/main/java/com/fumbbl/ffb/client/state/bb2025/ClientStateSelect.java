...
public class ClientStateSelect extends ClientStateAwt<SelectLogicModule> {
	...
	protected int menuIndex(ClientAction action) {
		if (actionMapping(0).containsValue(action)) {
			return 0;
		}
		return 1;
	}
		...
		if (menuIndex == 0) {
			...
			switch (pActionKey) {
				...
				case PLAYER_ACTION_CHOMP:
					menuItemSelected(selectedPlayer, IPlayerPopupMenuKeys.KEY_CHOMP);
					break;
				default:
					actionHandled = super.actionKeyPressed(pActionKey, menuIndex);
					break;
	...
	protected LinkedHashMap<ClientAction, MenuItemConfig> itemConfigs(ActionContext actionContext) {
		...
		configs.put(ClientAction.BOMB,
			new MenuItemConfig("Throw Bomb Action", IIconProperty.ACTION_BOMB, IPlayerPopupMenuKeys.KEY_BOMB));
		configs.put(ClientAction.SHOT_TO_NOTHING_BOMB, new MenuItemConfig("Shot To Nothing Bomb",
			IIconProperty.ACTION_BOMB,
		...
		configs.put(ClientAction.PASS,
			new MenuItemConfig("Pass Action", IIconProperty.ACTION_PASS, IPlayerPopupMenuKeys.KEY_PASS));
		configs.put(ClientAction.SHOT_TO_NOTHING,
			new MenuItemConfig("Shot To Nothing", IIconProperty.ACTION_PASS, IPlayerPopupMenuKeys.KEY_SHOT_TO_NOTHING));
		configs.put(ClientAction.HAND_OVER,
			new MenuItemConfig("Hand Over Action", IIconProperty.ACTION_HAND_OVER, IPlayerPopupMenuKeys.KEY_HAND_OVER));
		configs.put(ClientAction.THROW_TEAM_MATE, new MenuItemConfig("Throw Team-Mate Action", IIconProperty.ACTION_PASS,
			IPlayerPopupMenuKeys.KEY_THROW_TEAM_MATE));
		configs.put(ClientAction.KICK_TEAM_MATE,
			new MenuItemConfig("Kick Team-Mate Action", IIconProperty.ACTION_BLITZ,
				IPlayerPopupMenuKeys.KEY_KICK_TEAM_MATE));
		configs.put(ClientAction.BEER_BARREL_BASH,
			new MenuItemConfig("Beer Barrel Bash", IIconProperty.ACTION_BEER_BARREL_BASH,
				IPlayerPopupMenuKeys.KEY_BEER_BARREL_BASH));
		configs.put(ClientAction.ALL_YOU_CAN_EAT,
			new MenuItemConfig("All You Can Eat", IIconProperty.ACTION_ALL_YOU_CAN_EAT,
				IPlayerPopupMenuKeys.KEY_ALL_YOU_CAN_EAT));
		configs.put(ClientAction.KICK_EM_BLOCK,
			new MenuItemConfig("Kick 'em while they are down! (Block)", IIconProperty.ACTION_KICK_EM_BLOCK,
				IPlayerPopupMenuKeys.KEY_KICK_EM_BLOCK));
		configs.put(ClientAction.KICK_EM_BLITZ,
			new MenuItemConfig("Kick 'em while they are down! (Blitz)", IIconProperty.ACTION_KICK_EM_BLITZ,
				IPlayerPopupMenuKeys.KEY_KICK_EM_BLITZ));
		configs.put(ClientAction.THE_FLASHING_BLADE,
			new MenuItemConfig("The Flashing Blade", IIconProperty.ACTION_THE_FLASHING_BLADE,
				IPlayerPopupMenuKeys.KEY_THE_FLASHING_BLADE));
		configs.put(ClientAction.VICIOUS_VINES,
			new MenuItemConfig("Vicious Vines", IIconProperty.ACTION_VICIOUS_VINES, IPlayerPopupMenuKeys.KEY_VICIOUS_VINES));
		configs.put(ClientAction.FURIOUS_OUTBURST,
	...
	protected Map<Influences, Map<ClientAction, MenuItemConfig>> influencedItemConfigs() {
		...
		treacherous.put(ClientAction.PASS,
			new MenuItemConfig("Pass Action (Treacherous)", IIconProperty.ACTION_PASS, IPlayerPopupMenuKeys.KEY_PASS));
		treacherous.put(ClientAction.SHOT_TO_NOTHING,
			new MenuItemConfig("Shot To Nothing (Treacherous)", IIconProperty.ACTION_PASS,
