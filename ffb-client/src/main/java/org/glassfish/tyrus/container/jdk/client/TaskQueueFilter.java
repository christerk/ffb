 ...
 * {@link #close()} and {@link #startSsl()} method calls and passes them to a
 * downstream filter one at a time. Both
 * {@link org.glassfish.tyrus.container.jdk.client.SslFilter} and
 * {@link org.glassfish.tyrus.container.jdk.client.TransportFilter} allow
 * {@link #write(java.nio.ByteBuffer, org.glassfish.tyrus.spi.CompletionHandler)}
 * method call only after the previous one has completed. Queueing
 * {@link #close()} method calls ensures that
 * {@link #write(java.nio.ByteBuffer, org.glassfish.tyrus.spi.CompletionHandler)}
 * methods called before {@link #close()} will be processed. Including
 * {@link #startSsl()} methods in the queue ensures that no
 * {@link #write(java.nio.ByteBuffer, org.glassfish.tyrus.spi.CompletionHandler)}
 * method will be passed to
 * {@link org.glassfish.tyrus.container.jdk.client.SslFilter} while it performs
 * SSL handshake.
 *
...
class TaskQueueFilter extends Filter {
	 ...
	 * Constructor.
	 *
	 * @param downstreamFilter a filter that is positioned directly under this
	 *                         filter.
	 */
