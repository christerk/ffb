 ...
 * and {@link #onRead(java.nio.ByteBuffer)} calls are passed through until
 * {@link #startSsl()} method is called, after which SSL handshake is started.
 * When SSL handshake is in progress, no data passed in
 * {@link #write(java.nio.ByteBuffer, org.glassfish.tyrus.spi.CompletionHandler)}
 * are permitted until SSL handshake completes. After SSL handshake has
 * completed, all data passed in write method will be encrypted and data passed
 * in {@link #onRead(java.nio.ByteBuffer)} method will be decrypted.
 *
...
class SslFilter extends Filter {
	...
	/*
	 * SYNCHRONIZATION NOTE: SSLEngine#wrap and SSLEngine#unwrap can be done
	 * concurrently (one thread doing wrap and another doing unwrap). The same
	 * operation cannot be done concurrently (2 threads doing wrap).
	 * 
	 * Method doHandshakeStep must be synchronized, because it might be entered both
	 * by writing and reading thread during re-handshake. Write, close and
	 * re-handshake cannot be done concurrently, because all those operations might
	 * do SSLEngine#wrap. Read can be be done concurrently with any other operation,
	 * because even thought re-handshake can do SSLEngine#unwrap, it won't do so if
	 * it was entered from write operation.
	 * 
	 * Operations upstreamFilter#onRead cannot be done while holding a lock of this
	 * class. Doing so might lead to a deadlock. An example of deadlock would be if
	 * a thread holding a lock in upstreamFilter#onRead writes a response
	 * synchronously (blocks and waits for write completion handler). The write
	 * completion handler might be executed by another thread which will not be able
	 * to obtain a lock for this class.
	 */

	/*
	 * Some operations on SSL engine require a buffer as a parameter even if they
	 * don't need any data. This buffer is for that purpose.
	 */
	...
	/*
	 * Pending write operation stored when writing data was not possible. It will be
	 * resumed when write operation is available again. Only one write operation can
	 * be in progress at a time. Trying to store more than one pending application
	 * write indicates that an upper stack called write without waiting for the
	 * completion handler of the previous write. Currently this is used only during
	 * re-handshake.
	 */
	 ...
	 *
	 * @param downstreamFilter      a filter that is positioned under the SSL
	 *                              filter.
	 * @param sslEngineConfigurator configuration of SSL engine.
	 * @param serverHost            server host (hostname or IP address), which will
	 *                              be used to verify authenticity of the server
	 *                              (the provided host will be compared against the
	 *                              host in the certificate provided by the server).
	 *                              IP address and hostname cannot be used
	 *                              interchangeably - if a certificate contains
	 *                              hostname and an IP address of the server is
	 *                              provided here, the verification will fail.
	 */
	SslFilter(Filter downstreamFilter, SslEngineConfigurator sslEngineConfigurator, String serverHost) {
		...
		/**
		 * Enable server host verification. This can be moved to
		 * {@link SslEngineConfigurator} with the rest of {@link SSLEngine}
		 * configuration when {@link SslEngineConfigurator} supports Java 7.
		 */
	 ...
	 *             {@link #SslFilter(Filter, org.glassfish.tyrus.client.SslEngineConfigurator, String)}.
	 */
	...
	synchronized void write(final ByteBuffer applicationData, final CompletionHandler<ByteBuffer> completionHandler) {
		...
		/*
		 * TODO: The current model does not permit calling write before SSL handshake
		 * has completed, if we allow this we could easily get rid of the
		 * onSslHandshakeCompleted event. The SSL filter can simply store the write
		 * until the handshake has completed like during re-handshake. With such a
		 * change HANDSHAKING and REHANDSHAKING could be collapsed into one state.
		 */
		case HANDSHAKING: {
		...
		/*
		 * Suspend all writes until the re-handshaking is done. Data are permitted
		 * during re-handshake in SSL, but this would only complicate things
		 */
		case REHANDSHAKING: {
	...
	private void handleWrite(final ByteBuffer applicationData, final CompletionHandler<ByteBuffer> completionHandler) {
		try {
			// transport buffer always writes all data, so there are not leftovers in the
			// networkOutputBuffer
			...
			case BUFFER_OVERFLOW: {
				 ...
				 * networkOutputBuffer, we make sure to set networkOutputBuffer > max 16kB + SSL
				 * headers when initializing this filter. This indicates a bug.
				 */
				throw new IllegalStateException(
						"SSL packet does not fit into the network buffer: " + networkOutputBuffer + "\n" + getDebugState());
			}

			case BUFFER_UNDERFLOW: {
				/*
				 * This basically says that there is not enough data to create an SSL packet.
				 * Javadoc suggests that BUFFER_UNDERFLOW can occur only after unwrap(), but to
				 * be 100% sure we handle all possible error states:
				 */
				throw new IllegalStateException(
						"SSL engine underflow with the following application input: " + applicationData + "\n" + getDebugState());
			}
	...
	private void storePendingApplicationWrite(final ByteBuffer applicationData,
		...
		if (pendingApplicationWrite != null) {
			/*
			 * If this happens it means a bug in this class or upper layer called another
			 * write() without waiting for a completion handler of the previous one.
			 */
		...
		pendingApplicationWrite = new Runnable() {
			@Override
			public void run() {
				// go again through the entire write procedure like this data came directly from
				// the application
	...
	boolean processRead(ByteBuffer networkData) {
		/*
		 * A flag indicating if we should keep reading from the network buffer. If
		 * false, the buffer contains an uncompleted packet -> stop reading, SSL engine
		 * accepts only whole packets
		 */
	...
	private boolean handleRead(ByteBuffer networkData) {
		try {
			...
			case BUFFER_OVERFLOW: {
				/*
				 * This means that the content of the ssl packet (max 16kB) did not fit into
				 * applicationInputBuffer, but we make sure to set applicationInputBuffer > max
				 * 16kB when initializing this filter. This indicates a bug.
				 */
				throw new IllegalStateException(
						"Contents of a SSL packet did not fit into buffer: " + applicationInputBuffer + "\n" + getDebugState());
			}

			case BUFFER_UNDERFLOW: {
				// the ssl packet is not full, return and indicate that we won't get more from
				// this buffer
				return false;
			}
			...
			case OK: {
				...
				if (sslEngine.isInboundDone()) {
					/*
					 * we have just received a close alert from our peer, so we are done. If there
					 * is something remaining in the input buffer, just drop it.
					 */
	...
	private boolean doHandshakeStep(ByteBuffer networkData) {
		 ...
		 * Application data can be interleaved with handshake messages only during
		 * re-handshake. We don't use applicationInputBuffer, because we might want to
		 * store more than one packet
		 */
		...
		synchronized (this) {
			...
			try {
				/*
				 * we don't use networkOutputBuffer, because there might be a write operation
				 * still in progress -> we don't want to corrupt the buffer it is using
				 */
				...
				while (!stepFinished) {
					...
					case NOT_HANDSHAKING: {
						 ...
						 * in the state machine of this class, because we stopped handshaking and did
						 * not exit this while loop. The could be caused either by overlooking FINISHED
						 * state or incorrectly treating an error.
						 */
					...
					case FINISHED: {
						/*
						 * According to SSLEngine javadoc FINISHED status can be returned only in
						 * SSLEngineResult, but just to make sure we don't end up in an infinite loop
						 * when presented with an SSLEngine implementation that does not respect this:
						 */
						stepFinished = true;
					...
					case NEED_WRAP: {
						...
						case BUFFER_UNDERFLOW: {
							 ...
							 */
							throw new IllegalStateException(
									"SSL engine underflow with the following SSL filter " + "state: \n" + getDebugState());
						}
		...
		/*
		 * Handle any read data. We have to execute upstreamFilter.onRead after
		 * releasing the lock. See the synchronization note on top. Only one read
		 * operation can be in progress at a time, so even though we have released the
		 * lock, no other SSlEngine#unwrap can be performed until this method returns.
		 * So there is no chance of the read data being mixed up
		 */
		...
		/*
		 * if we are here, it means that we are waiting for more data -> indicate that
		 * there is nothing usable in the input buffer left
		 */
		return false;
	}

	private void handleHandshakeFinished() {
		// Apply a custom host verifier if present. Do it for both handshaking and
		// re-handshaking.
		if (customHostnameVerifier != null && !customHostnameVerifier.verify(serverHost, sslEngine.getSession())) {
			handleSslError(
					new SSLException("Server host name verification using " + customHostnameVerifier.getClass() + " has failed"));
			return;
		}
		...
		if (state == State.HANDSHAKING) {
			...
			if (pendingApplicationWrite != null) {
				...
				// set pending write to null to cover the extremely improbable case that we
				// start re-handshaking again
	...
	/**
	 * Returns a printed current state of the SslFilter that could be helpful for
	 * troubleshooting.
	 */
	...
	private enum State {
		NOT_STARTED, HANDSHAKING, REHANDSHAKING, DATA, CLOSED
	}
	...
	// synchronized on the outer class, because there is a danger of deadlock if
	...
	private class WriteQueue {
		...
		void write(final ByteBuffer data, final CompletionHandler<ByteBuffer> completionHandler) {
			synchronized (SslFilter.this) {
				...
				// if our task is the first one in the queue, there is no other write task in
				// progress -> process it
		...
		public String toString() {
			synchronized (SslFilter.this) {
				return "WriteQueue{" + "pendingWrites=" + pendingWrites.size() + '}';
			}
		}
	}
}