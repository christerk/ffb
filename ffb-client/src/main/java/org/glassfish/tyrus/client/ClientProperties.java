...
public final class ClientProperties {
	 ...
	 * is currently ignored but must be present ({@link java.net.URI#URI(String)} is
	 * used for parsing).
	 * 
	 * <pre>
	 ...
	 * <p>
	 * Value is expected to be {@link java.util.Map}&lt;{@link String},
	 * {@link String}&gt; and represent raw http headers to be added to initial
	 * request which is sent to proxy. Key corresponds to header name, value is
	 * header value.
	 * <p>
	...
	/**
	 * Property usable in {@link ClientManager#getProperties()} as a key for SSL
	 * configuration.
	 * <p>
	 * Value is expected to be either
	 * {@code org.glassfish.grizzly.ssl.SSLEngineConfigurator} or
	 * {@link org.glassfish.tyrus.client.SslEngineConfigurator} when configuring
	 * Grizzly client or only
	 * {@link org.glassfish.tyrus.client.SslEngineConfigurator} when configuring JDK
	 * client.
	 * <p>
	 * The advantage of using
	 * {@link org.glassfish.tyrus.client.SslEngineConfigurator} with Grizzly client
	 * is that {@link org.glassfish.tyrus.client.SslEngineConfigurator} allows
	 * configuration of host name verification (which is turned on by default)
	 * <p>
	 ...
	 * {@link org.glassfish.tyrus.spi.ClientContainer#openClientSocket(javax.websocket.ClientEndpointConfig, java.util.Map, org.glassfish.tyrus.spi.ClientEngine)}.
	 */
	public static final String INCOMING_BUFFER_SIZE = "org.glassfish.tyrus.incomingBufferSize";

	/**
	 * When set to {@code true} (boolean value), client runtime preserves used
	 * container and reuses it for outgoing connections.
	 * <p>
	 * A single thread pool is reused by all clients with this property set to
	 * {@code true}. JDK client supports only shared container option, so setting
	 ...
	 * <p>
	 * An instance of {@link org.glassfish.tyrus.client.ThreadPoolConfig} is
	 * expected for both JDK and Grizzly client. Instance of
	 * {@code org.glassfish.grizzly.threadpool.ThreadPoolConfig}, can be used for
	 * Grizzly client.
	 * <p>
	...
	/**
	 * Authentication configuration. If no AuthConfig is specified then default
	 * configuration will be used, containing both Basic and Digest provided
	 * authenticators.
	 * <p>
	 ...
	 * <p>
	 * When set to {@code true} and one of the following redirection HTTP response
	 * status code (3xx) is received during a handshake, client will attempt to
	 * connect to the {@link URI} contained in {@value UpgradeResponse#LOCATION}
	 * header from handshake response. Number of redirection is limited by property
	 * {@link #REDIRECT_THRESHOLD} (integer value), while default value is
	 ...
	 * <p>
	 * Value is expected to be positive {@link Integer}. Default value is
	 * {@value TyrusClientEngine#DEFAULT_REDIRECT_THRESHOLD}.
	 * <p>
	 * HTTP redirection must be enabled by property {@link #REDIRECT_ENABLED},
	 * otherwise {@code REDIRECT_THRESHOLD} is not applied.
	 *
	...
	/**
	 * HTTP Service Unavailable - {@value UpgradeResponse#RETRY_AFTER} reconnect
	 * support.
	 * <p>
	 * Value is expected to be {@code boolean}. Default value is {@code false}.
	 * <p>
	 * When set to {@code true} and HTTP response code
	 * {@code 503 - Service Unavailable} is received, client will attempt to
	 * reconnect after delay specified in {@value UpgradeResponse#RETRY_AFTER}
	 * header from handshake response. According to RFC 2616 the value must be
	 * decimal integer (representing delay in seconds) or {@code
	 * http-date}.
	 * <p>
	 * Tyrus client will try to reconnect after this delay if:
	 * <ul>
	 * <li>{@value UpgradeResponse#RETRY_AFTER} header is present and is not
	 * empty</li>
	 ...
	 * Value is expected to be {@code boolean}. Default value is {@code false}.
	 * <p>
	 * When set to {@code true} upgrade request and response messages will be logged
	 * regardless of the logging level configuration. When the logging is configured
	 * to {@link java.util.logging.Level#FINE} or lower, this setting will have no
	 * effect as at this level HTTP upgrade messages will be logged anyway.
	 */
	public static final String LOG_HTTP_UPGRADE = "org.glassfish.tyrus.client.http.logUpgrade";

	/**
	 * Property name for registering a custom masking key generator. The expected
	 * value is an instance of {@link org.glassfish.tyrus.core.MaskingKeyGenerator}.
	 * <p>
	 * As a security measure, all frames originating on websocket client have to be
	 * masked with random 4B value, which should be freshly generated for each
	 * frame. Moreover to fully comply with the security requirements of RFC 6455, a
	 * masking key of a frame must not be predictable from masking keys of previous
	 * frames and therefore Tyrus uses {@link java.security.SecureRandom} as a
	 * default masking key generator. While this is perfectly OK for most Tyrus
	 * client use cases, usage of {@link java.security.SecureRandom} might prove to
	 * be a performance issue, when the client is used for instance for highly
	 * parallel stress testing as {@link java.security.SecureRandom} uses a
	 * synchronized singleton as a random entropy provider in its internals.
	 * <p>
	 * This property allows replacing the default {@link java.security.SecureRandom}
	 * with a more scalable provider of masking keys.
	 */
