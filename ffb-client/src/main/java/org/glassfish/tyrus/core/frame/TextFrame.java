...
public class TextFrame extends TyrusFrame {
	 ...
	 * Constructor.
	 *
	 * @param message      text message (will be encoded using strict UTF-8
	 *                     encoding).
	 * @param continuation {@code true} when this frame is continuation frame,
	 *                     {@code false} otherwise.
	...
	public TextFrame(String message, boolean continuation, boolean fin) {
		super(Frame.builder().payloadData(encode(new StrictUtf8(), message))
				.opcode(continuation ? (byte) 0x00 : (byte) 0x01).fin(fin).build(),
				continuation ? FrameType.TEXT_CONTINUATION : FrameType.TEXT);
		this.continuation = continuation;
	 ...
	 * <p>
	 * This might be removed in the future, if encoding part will be separated from
	 * text frame impl.
	 *
	 * @return UTF-8 decoding remainder. Used internally to decoding next incoming
	 *         frame.
	 */
