 ...
 * <p>
 * Utility class, which helps to configure {@link SSLEngine}. Should be passed
 * to client via configuration properties. Example:
 * 
 * <pre>
...
public class SslEngineConfigurator {
	...
	/**
	 * Create SSL Engine configuration based on passed {@link SSLContext}, using
	 * passed client mode, need/want client auth parameters.
	 *
	 * @param sslContext     {@link SSLContext}.
	 ...
	 * {@link SslContextConfigurator}. This constructor makes possible to initialize
	 * SSLEngine and SSLContext in lazy fashion on first
	 * {@link #createSSLEngine(String)} call.
	 *
	 ...
	 *
	 * @param serverHost server host, which will be used to verify authenticity of
	 *                   the server (the provided host name will compared to the
	...
	public SSLEngine createSSLEngine(String serverHost) {
		...
		/*
		 * the port is not part of host name verification, it is present in the
		 * constructor because of Kerberos (which is not supported by Tyrus)
		 */
	 ...
	 * excluding Hostname Verification.
	 *
	 ...
	 *
	 * @return <tt>true</tt>, if {@link SSLEngine} will be configured to work in
	 *         <tt>client</tt> mode, or <tt>false</tt> for <tt>server</tt> mode.
	 */
	 ...
	 *
	 * @param clientMode <tt>true</tt>, if {@link SSLEngine} will be configured to
	 *                   work in <tt>client</tt> mode, or <tt>false</tt> for
	 *                   <tt>server</tt> mode.
	 ...
	 *
	 * @return {@code true} if the hostname verification is enabled, {@code false}
	 *         otherwise.
	 */
	 ...
	 *
	 * @param hostVerificationEnabled when {@code true}, servers hostname will be
	 *                                verified using JDK default
	 *                                {@link HostnameVerifier}. When {@code false},
	 *                                hostname verification won't be performed
	 *                                unless custom {@link HostnameVerifier} is set.
	 * @return updated {@link SslEngineConfigurator}.
	 ...
	 * <p>
	 * When custom {@link HostnameVerifier} instance is registered, it will be used
	 * to perform hostname verification, no matter on the state of hostname
	 * verification flag (see {@link #isHostVerificationEnabled()}) and JDK default
	...
	private static String[] configureEnabledProtocols(SSLEngine sslEngine, String[] requestedProtocols) {
		...
		for (String supportedProtocol : supportedProtocols) {
			/*
			 * Check to see if the requested protocol is among the supported protocols,
			 * i.e., may be enabled
			 */
