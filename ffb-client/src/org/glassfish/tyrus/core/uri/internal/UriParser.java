 ...
 * {@link java.net.URI URIs} from Strings. Example of parsed uri:
 * {@code "http://user@{host}:{port}/a/{path}?query=1#fragment"}. The parser is
 * not thread safe.
 *
 * @author Paul Sandoz
...
class UriParser {
	...
	/**
	 * Parses the URI component. Parsing starts at position of the first character
	 * of component and ends with position of one of the delimiters. The string and
	 * current position is taken from the {@link CharacterIterator}.
	 *
	...
	/**
	 * Parses the input string URI. After calling this method The result components
	 * can be retrieved by calling appropriate getter methods like
	 * {@link #getHost()}, {@link #getPort()}, etc.
	 */
	public void parse() {
		...
		if (ci.current() == ':') {
			// absolute
			if (comp == null) {
				throw new IllegalArgumentException(String.format("Expected scheme name at index %d: \"%s\"", ci.pos(), input));
			}
			scheme = comp;
			if (!ci.hasNext()) {
	...
	/**
	 * Returns parsed scheme specific part. The {@link #parse() method} must be
	...
	/**
	 * Returns parsed scheme component. The {@link #parse() method} must be called
	...
	/**
	 * Returns parsed user info component. The {@link #parse() method} must be
	 * called before executing this method.
	 *
	 * @return User info.
	 */
	...
	/**
	 * Returns parsed host component. The {@link #parse() method} must be called
	 * before executing this method.
	 *
	 * @return Host.
	 */
	...
	/**
	 * Returns parsed port component. The {@link #parse() method} must be called
	 * before executing this method.
	 *
	 * @return Port.
	 */
	...
	/**
	 * Returns parsed query component. The {@link #parse() method} must be called
	 * before executing this method.
	 *
	 * @return Query.
	 */
	...
	/**
	 * Returns parsed path component. The {@link #parse() method} must be called
	 * before executing this method.
	 *
	 * @return Path.
	 */
	...
	/**
	 * Returns parsed fragment component. The {@link #parse() method} must be called
	 * before executing this method.
	 *
	 * @return Fragment.
	 */
	...
	/**
	 * Returns parsed authority component. The {@link #parse() method} must be
	 * called before executing this method.
	 *
	 * @return Authority.
	 */
	...
	/**
	 * Returns whether the input string URI is opaque. The {@link #parse() method}
