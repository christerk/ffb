...
public class Frame {
	 ...
	 * Get masking key.
	 *
	 * @return masking key (32 bit value) or {@code null} when the frame should not
	 *         be masked.
	 */
	 ...
	 * Get payload data.
	 * <p>
	 * Changes done to returned array won't be propagated to current {@link Frame}
	 * instance. If you need to modify payload, you have to create new instance, see
	 * {@code Builder#Frame(Frame)}. Length of returned array will be always same as
	 ...
	 *
	 * @return {@code true} when this frame is control (close, ping, pong) frame,
	...
	public static final class Builder {
		 ...
		 * Set payload length.
		 * <p>
		 * Payload length is automatically set to payloadData length when
		 * {@link #payloadData(byte[])} is called. This method can limit the data used
		 * for this frame by setting smaller value than payloadData.length.
		 *
		 * @param payloadLength payload length. Must not be greater than
		 *                      payloadData.length.
		 * @return updated {@link Builder} instance.
		...
		/**
		 * Set payload data. {@link #payloadLength(long)} is also updated with
		 * payloadData.length.
		 *
