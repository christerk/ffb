...
/**
 * Implementation of all possible request interfaces. Should be the only point
 * of truth.
 *
...
public final class RequestContext extends UpgradeRequest {
	...
	private RequestContext(URI requestURI, String queryString, Object httpSession, boolean secure,
			Principal userPrincipal, Builder.IsUserInRoleDelegate IsUserInRoleDelegate, String remoteAddr,
	 ...
	 * Get headers.
	 *
	 * @return headers map. List items are corresponding to header declaration in
	 *         HTTP request.
	 */
	@Override
	...
	public static final class Builder {
		 ...
		 *
		 * @param isUserInRoleDelegate delegate for
		 *                             {@link RequestContext#isUserInRole(String)}.
		 * @return updated {@link RequestContext.Builder} instance.
		 */
		 ...
		 * Set parameter map.
		 *
		 * @param parameterMap parameter map. Takes map returned from
		...
		public RequestContext build() {
			return new RequestContext(requestURI, queryString, httpSession, secure, userPrincipal, isUserInRoleDelegate,
					remoteAddr, parameterMap != null ? parameterMap : new HashMap<String, List<String>>(), headers);
		}

		/**
		 * Is user in role delegate.
		 * <p>
		 * Cannot easily query ServletContext or HttpServletRequest for this
		 * information, since it is stored only as object.
		 */
		public interface IsUserInRoleDelegate {

			/**
			 * Returns a boolean indicating whether the authenticated user is included in
			 * the specified logical "role". Roles and role membership can be defined using
			 ...
			 * @return a boolean indicating whether the user making this request belongs to
			 *         a given role; false if the user has not been authenticated.
			 */
			public boolean isUserInRole(String role);
		}
	}
