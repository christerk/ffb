...
public class ReportKickoffThrowARock implements IReport {
	...
	public void addStats(Game game, List<DieStat<?>> diceStats) {
		diceStats.add(new DicePoolStat(DieBase.D6, TeamMapping.TEAM, game.getTeamHome().getId(), Collections.singletonList(fRollHome), false));
		diceStats.add(new DicePoolStat(DieBase.D6, TeamMapping.TEAM, game.getTeamAway().getId(), Collections.singletonList(fRollAway), false));
	}
}
