...
public class UtilPassing {
  ...
  public static Player[] findInterceptors(Game pGame, Player pThrower, FieldCoordinate pTargetCoordinate) {
    ...
    if ((pTargetCoordinate != null) && (pThrower != null)) {
      ...
      for (int i = 0; i < otherPlayers.length; i++) {
        ...
        if ((interceptorCoordinate != null) && (interceptorState != null) && interceptorState.hasTacklezones() && !UtilCards.hasSkill(pGame, otherPlayers[i], Skill.NO_HANDS)) {
  ...
  public static Set<FieldCoordinate> findValidPassBlockEndCoordinates(Game pGame) {
	  ...
	  ActingPlayer actingPlayer = pGame.getActingPlayer();
		...
		FieldCoordinate[] neighbours = pGame.getFieldModel().findAdjacentCoordinates(pGame.getFieldModel().getPlayerCoordinate(pGame.getThrower()), FieldCoordinateBounds.FIELD, 1, false);
