...
public class FileIterator implements Iterator<File> {
   ...
   * @param pStartDirectory directory structure starting point.
   * @param pIncludeDirectories whether directories should be returned as well
   * @param pFileFilter filter to be applied to all files and
  ...
  public FileIterator(File pStartDirectory, boolean pIncludeDirectories, FileFilter pFileFilter) {
  	if (pFileFilter == null) {	setFileFilter(ACCEPT_ALL); } else { setFileFilter(pFileFilter); }
  ...
  protected void descend() {    
    ...
    while (directoryIterator.hasNext()) {
      ...
      if (fIncludeDirectories) { nextLevelFiles.add(directory); }
      File[] files = directory.listFiles();
      if (files != null) {
		    for (int i = 0; i < files.length; i++) {
	        if (files[i].isFile()) {
	          if (fFileFilter.accept(files[i])) { nextLevelFiles.add(files[i]); }
  ...
  public boolean hasNext() {
    while ((fFileIterator == null) || (!fFileIterator.hasNext() && (fDirectoryList.size() > 0))) { descend(); }
