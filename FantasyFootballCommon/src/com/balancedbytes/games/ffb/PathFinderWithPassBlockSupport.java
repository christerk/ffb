...
public class PathFinderWithPassBlockSupport {
  ...
  /**
   * Gets the shortest path from the player in the start square to the end
   * square. The start square must contain a player. The path will not leave
   * or pass through a tackle zone, but it may end in one. The path will also
   * avoid going through or landing on a player.
   * 
   * @param fieldModel
  ...
  public static FieldCoordinate[] getShortestPath(Game pGame, FieldCoordinate pEndCoord) {
    ...
    FieldModel fieldModel = pGame.getFieldModel();
		...
		while (openSet.size() > 0) {
			...
			FieldCoordinate[] neighbours = fieldModel.findAdjacentCoordinates(current.coord, FieldCoordinateBounds.FIELD, searchDistance, false);
			for (FieldCoordinate neighbourCoord : neighbours) {

			  int distance = current.coord.distanceInSteps(neighbourCoord);

			  // Don't allow a leap if the context explicitly disallows it, if the path already has lept before or if the player can't leap
			  if (distance > 1 && (maxDistance-current.distance-distance < 0 || current.state == leapState || !context.allowLeap || !canLeap))
			    continue;
			  ...
			  neighbour = data.getNeighbour(neighbourState, neighbourCoord);
				...
				if (neighbour != null && (closedSet.contains(neighbour)	|| (neighbour.tz && !pEndCoords.contains(neighbourCoord)))) {
					continue;
				}
	...
	public static FieldCoordinate[] allowPassBlockMove(Game pGame, Player passBlocker, FieldCoordinate startPosition, int distance, boolean canLeap) {
	  ...
	  if (!UtilCards.hasSkill(pGame, passBlocker, Skill.PASS_BLOCK)) {
		...
		FieldCoordinate[] path = getShortestPath(pGame, startPosition, validEndCoordinates, distance, passBlocker.getTeam(), passBlockContext, canLeap);
