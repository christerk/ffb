			...
			if (doNextStep) {
				...
				switch (fInjuryResult.injuryContext().getApothecaryStatus()) {
					...
					case USE_IGOR:
						...
						inducementSetIgor.getInducementMapping().keySet().stream().filter(type -> type.hasSingleUsage(Usage.REGENERATION))
					...
					default:
						...
						if ((playerState != null) && playerState.isCasualty()
							...
							if (!UtilServerInjury.handleRegeneration(this, player)) {
								...
								boolean hasInducement = inducementSet.getInducementMapping().keySet().stream().anyMatch(type -> type.hasSingleUsage(Usage.REGENERATION)
