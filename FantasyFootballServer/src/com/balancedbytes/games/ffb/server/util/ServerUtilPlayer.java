...
public class ServerUtilPlayer {
  ...
  public static int findBlockStrength(Game game, Player attacker, int attackerStrength, Player defender) {
    ...
    boolean flipOpponentIfSameTeam = UtilServerCards.hasSkillWithProperty(attacker, NamedProperties.flipSameTeamOpponentToOtherTeam);
    ...
    Player[] offensiveAssists = UtilPlayer.findAdjacentPlayersWithTacklezones(game, attacker.getTeam(), coordinateDefender, false);
    for (int i = 0; i < offensiveAssists.length; i++) {
      if (offensiveAssists[i] != attacker) {
        ...
        Player[] defensiveAssists = UtilPlayer.findAdjacentPlayersWithTacklezones(game, defenderTeam, coordinateAssist, false);        
