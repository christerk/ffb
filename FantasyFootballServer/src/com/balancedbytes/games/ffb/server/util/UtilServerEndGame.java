 ...
 * @author Kalimar
 */
public class UtilServerEndGame {
  ...
  public static ReportWinningsRoll rollWinnings(GameState pGameState, boolean pReRolled) {
    ...
    if (!pReRolled) {
      if (scoreDiffHome > 0) {
        UtilServerDialog.showDialog(pGameState, new DialogWinningsReRollParameter(game.getTeamHome().getId(), rollHome));
      }
      if (scoreDiffHome < 0) {
        UtilServerDialog.showDialog(pGameState, new DialogWinningsReRollParameter(game.getTeamAway().getId(), rollAway));
      }
    }
