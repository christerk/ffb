...
import com.balancedbytes.games.ffb.server.db.IDbTableInducements;
 ...
 * @author Kalimar
 */
public class DbInducementsDelete extends DbUpdateStatement {
  
  private PreparedStatement fStatement;
  
  public DbInducementsDelete(FantasyFootballServer pServer) {
    super(pServer);
  }
  
  public DbStatementId getId() {
    return DbStatementId.INDUCEMENTS_DELETE;
  }
  
  public void prepare(Connection pConnection) {
    try {
      StringBuilder sql = new StringBuilder();
      sql.append("DELETE FROM ").append(IDbTableInducements.TABLE_NAME).append(" WHERE ").append(IDbTableInducements.COLUMN_GAME_STATE_ID).append("=?");
  ...
  private PreparedStatement fillDbStatement(IDbUpdateParameter pUpdateParameter) throws SQLException {
    DbInducementsDeleteParameter parameter = (DbInducementsDeleteParameter) pUpdateParameter;
