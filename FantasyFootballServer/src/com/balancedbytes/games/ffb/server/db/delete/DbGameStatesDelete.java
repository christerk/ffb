 ...
 * @author Kalimar
 */
public class DbGameStatesDelete extends DbUpdateStatement {
  
  private PreparedStatement fStatement;
  
  public DbGameStatesDelete(FantasyFootballServer pServer) {
    super(pServer);
  }
  
  public DbStatementId getId() {
    return DbStatementId.GAME_STATES_DELETE;
  }
  
  public void prepare(Connection pConnection) {
    try {
      StringBuilder sql = new StringBuilder();
      sql.append("DELETE FROM ").append(IDbTableGameStates.TABLE_NAME).append(" WHERE ").append(IDbTableGameStates.COLUMN_ID).append("=?");
  ...
  private PreparedStatement fillDbStatement(IDbUpdateParameter pUpdateParameter) throws SQLException {
    DbGameStatesDeleteParameter parameter = (DbGameStatesDeleteParameter) pUpdateParameter;
