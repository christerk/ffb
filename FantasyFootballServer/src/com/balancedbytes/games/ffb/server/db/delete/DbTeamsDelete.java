 ...
 * @author Kalimar
 */
public class DbTeamsDelete extends DbUpdateStatement {
  
  private PreparedStatement fStatement;
  
  public DbTeamsDelete(FantasyFootballServer pServer) {
    super(pServer);
  }
  
  public DbStatementId getId() {
    return DbStatementId.TEAMS_DELETE;
  }
  
  public void prepare(Connection pConnection) {
    try {
      StringBuilder sql = new StringBuilder();
      sql.append("DELETE FROM ").append(IDbTableTeams.TABLE_NAME).append(" WHERE ").append(IDbTableTeams.COLUMN_GAME_STATE_ID).append("=?");
  ...
  private PreparedStatement fillDbStatement(IDbUpdateParameter pUpdateParameter) throws SQLException {
    DbTeamsDeleteParameter parameter = (DbTeamsDeleteParameter) pUpdateParameter;
