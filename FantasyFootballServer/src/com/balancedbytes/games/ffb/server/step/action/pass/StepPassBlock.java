 ...
 *   (parameter is consumed on TurnMode.PASS_BLOCK)
...
public class StepPassBlock extends AbstractStep {
  ...
  private void executeStep() {
  	...
  	if (game.getTurnMode().isBombTurn() || (game.getThrowerAction() == PlayerAction.DUMP_OFF) || (game.getThrowerAction() == PlayerAction.HAND_OVER) || (game.getThrowerAction() == PlayerAction.HAND_OVER_MOVE)) {
      	...
      	if (fOldTurnMode != TurnMode.DUMP_OFF) {
	    	...
	    	for (int i = 0; i < players.length; i++) {
	    		...
	    		if ((playerPosition != null) && !playerPosition.isBoxCoordinate()) {
	    			...
	    			game.getFieldModel().setPlayerState(players[i], playerState.changeActive(availablePassBlockers.contains(players[i])));
	    		}
	    	}
  ...
  private Set<Player> findPassBlockers(Team pTeam, boolean pCheckCanReach) {
      ...
      if (UtilCards.hasSkill(game, player, Skill.PASS_BLOCK)) {
        ...
        FieldCoordinate startPosition = game.getFieldModel().getPlayerCoordinate(player);
      	if (!pCheckCanReach || (playerState.hasTacklezones() && ArrayTool.isProvided(PathFinderWithPassBlockSupport.allowPassBlockMove(game, player, startPosition, 3, UtilCards.hasSkill(game, player, Skill.LEAP))))) {
  ...
  public void addTo(ByteList pByteList) {
  	...
  	if (fOldPlayerStates != null) {
  		pByteList.addByte((byte) fOldPlayerStates.length);
  		for (int i = 0; i < fOldPlayerStates.length; i++) {
  			if (fOldPlayerStates[i] != null) {
  				pByteList.addSmallInt(fOldPlayerStates[i].getId());
  			} else {
  				pByteList.addSmallInt(0);
  			}
  		}
  	} else {
