 ...
 * 
 * Expects stepParameter KICKOFF_BOUNDS to be set by a preceding step.
 * Expects stepParameter KICKOFF_RESULT to be set by a preceding step.
...
public final class StepApplyKickoffResult extends AbstractStep {
  ...
  private void handleRiot() {
    ...
    int riotRoll = 0;
    int turnModifier = 0;
    if ((game.isHomePlaying() && (turnDataAway.getTurnNr() == 0)) || (!game.isHomePlaying() && (turnDataHome.getTurnNr() == 0))) {
      turnModifier = 1;
    }
    if ((game.isHomePlaying() && (turnDataAway.getTurnNr() == 7)) || (!game.isHomePlaying() && (turnDataHome.getTurnNr() == 7))) {
      turnModifier = -1;
    }
    if (turnModifier == 0) {
  ...
  private void handleExtraReRoll() {
    ...
    getResult().addReport(new ReportKickoffExtraReRoll(fKickoffResult, rollHome, homeGainsReRoll, rollAway, awayGainsReRoll));
    getResult().setNextAction(StepAction.NEXT_STEP);
  ...
  private void handleThrowARock() {
    ...
    if (hitPlayerIdHome != null) {
      ...
      publishParameter(new StepParameter(StepParameterKey.INJURY_RESULT,
        	UtilServerInjury.handleInjury(this, new InjuryTypeThrowARock(this), null, player, playerCoordinate, null, ApothecaryMode.HOME)));
    ...
    if (hitPlayerIdAway != null) {
      ...
      publishParameter(new StepParameter(StepParameterKey.INJURY_RESULT,
      	UtilServerInjury.handleInjury(this, new InjuryTypeThrowARock(this), null, player, playerCoordinate, null, ApothecaryMode.AWAY)));
