		...
		registerModifier(new StepModifier<StepDropFallingPlayers, StepDropFallingPlayers.StepState>() {
			...
			public boolean handleExecuteStepHook(StepDropFallingPlayers step, StepState state) {
				 boolean doNextStep = true;
				    ...
				    if (((defenderState != null) && (defenderState.getBase() == PlayerState.FALLING) && (defenderCoordinate != null)) || (state.usingPilingOn != null)) {
				      if (state.usingPilingOn != null) {
				        ...
				        if (state.usingPilingOn && (!usesATeamReroll || UtilServerReRoll.useReRoll(step, ReRollSources.TEAM_RE_ROLL, actingPlayer.getPlayer()))) {
				          ...
				          if (rolledDouble && UtilGameOption.isOptionEnabled(game, GameOptionId.PILING_ON_TO_KO_ON_DOUBLE)) {
				        	  step.publishParameter(new StepParameter(StepParameterKey.INJURY_RESULT, UtilServerInjury.handleInjury(step, new InjuryTypePilingOnKnockedOut(step), null,
				        ...
				        state.injuryResultDefender = UtilServerInjury.handleInjury(step, injuryType, actingPlayer.getPlayer(), game.getDefender(), defenderCoordinate,
				          null, ApothecaryMode.DEFENDER);

				        if (UtilCards.hasSkillWithProperty(actingPlayer.getPlayer(), NamedProperties.appliesPoisonOnBadlyHurt) && state.injuryResultDefender.injuryContext().isBadlyHurt()) {
				        ...
				        if ((attackerState.getBase() != PlayerState.FALLING)
				            ...
				            && (!usesATeamReroll || UtilServerReRoll.isTeamReRollAvailable(step.getGameState(), actingPlayer.getPlayer()))
				            && attackerCoordinate.isAdjacent(defenderCoordinate)
				            && !state.injuryResultDefender.injuryContext().isCasualty()
				            && !attackerState.isRooted()
				            && (!UtilGameOption.isOptionEnabled(game, GameOptionId.PILING_ON_INJURY_ONLY) || state.injuryResultDefender.injuryContext().isArmorBroken())
				            && (!UtilGameOption.isOptionEnabled(game, GameOptionId.PILING_ON_ARMOR_ONLY) || !state.injuryResultDefender.injuryContext().isArmorBroken())
				            && (!UtilCards.hasCard(game, game.getDefender(), Card.BELT_OF_INVULNERABILITY) || state.injuryResultDefender.injuryContext().isArmorBroken())
				          ...
				          UtilServerDialog.showDialog(step.getGameState(), new DialogPilingOnParameter(actingPlayer.getPlayerId(), state.injuryResultDefender.injuryContext().isArmorBroken(),
