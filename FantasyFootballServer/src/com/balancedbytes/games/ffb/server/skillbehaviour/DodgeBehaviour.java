	...
	public DodgeBehaviour() {
			...
			public boolean handleExecuteStepHook(StepBlockDodge step, StepState state) {
				 findDodgeChoice(step, state);
				    ...
				    if (state.usingDodge == null) {
				      ...
				      step.getResult().addReport(new ReportSkillUse(game.getDefenderId(), skill, state.usingDodge, SkillUse.AVOID_FALLING));
				      if (state.usingDodge) {
	  ...
	  private void findDodgeChoice(StepBlockDodge step, StepState state) {
		    ...
		    if (state.usingDodge == null) {
		      ...
		      if ((actingPlayer.getPlayerAction() == PlayerAction.BLOCK) && UtilCards.hasSkillWithProperty(attacker, NamedProperties.canPushBackToAnySquare)
