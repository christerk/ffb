...
public class JsonObject extends JsonValue implements Iterable<Member> {
	...
	/**
	 * Adds a new member at the end of this object, with the specified name and the
	 ...
	 * member with a name that already exists in the object will add another member
	 * with the same name. In order to replace existing members, use the method
	 * <code>set(name, value)</code> instead. However, <strong> <em>add</em> is much
	 * faster than <em>set</em></strong> (because it does not need to search for
	 * existing members). Therefore <em>add</em> should be preferred when
