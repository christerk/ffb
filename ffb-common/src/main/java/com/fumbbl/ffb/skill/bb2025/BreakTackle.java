...
public class BreakTackle extends Skill {
	...
	public void postConstruct() {
		registerModifier(
			new DodgeModifier("Break Tackle ST 5+", "Break Tackle", -3, ModifierType.REGULAR, true, true) {
				@Override
		...
		registerModifier(
			new DodgeModifier("Break Tackle ST 4", "Break Tackle", -2, ModifierType.REGULAR, true, true) {
				@Override
				public boolean appliesToContext(Skill skill, DodgeContext context) {
					return context.getPlayer().getStrengthWithModifiers() == 4 && context.isSkillSelected(skill);
				}
			});
		registerModifier(
			new DodgeModifier("Break Tackle ST 3-","Break Tackle", -1, ModifierType.REGULAR, true, true) {
				@Override
				public boolean appliesToContext(Skill skill, DodgeContext context) {
					return context.getPlayer().getStrengthWithModifiers() <= 3 && context.isSkillSelected(skill);
				}
			});
	}
