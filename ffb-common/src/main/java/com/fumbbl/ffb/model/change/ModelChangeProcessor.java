	...
	public boolean apply(Game pGame, ModelChange pModelChange) {
		...
		switch (pModelChange.getChangeId()) {
			...
			case ACTING_PLAYER_SET_HAS_TRIGGERED_EFFECT:
				pGame.getActingPlayer().setHasTriggeredEffect((Boolean) pModelChange.getValue());
				return true;
			...
			case ACTING_PLAYER_SET_FELL_FROM_RUSH:
				...
				return true;
			case ACTING_PLAYER_SET_ILL_CARRY_YOU_AVAILABLE:
				pGame.getActingPlayer().setIllCarryYouAvailable((Boolean) pModelChange.getValue());
				return true;
			...
			case TURN_DATA_SET_KTM_USED:
				...
				return true;
			case TURN_DATA_SET_SECURE_THE_BALL_USED:
				getTurnData(pGame, isHomeData(pModelChange)).setSecureTheBallUsed((Boolean) pModelChange.getValue());
				return true;
			case TURN_DATA_SET_PUNT_USED:
				getTurnData(pGame, isHomeData(pModelChange)).setPuntUsed((Boolean) pModelChange.getValue());
				return true;
