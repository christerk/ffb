...
public class StepSwoop extends AbstractStep {
	...
	private void executeStep() {
		...
		if (state.usingSwoop == null) {
			UtilServerDialog.showDialog(gameState,
				new DialogSkillUseParameter(thrownPlayer.getId(),
					thrownPlayer.getSkillWithProperty(NamedProperties.ttmScattersInSingleDirection), 0),
				!game.getActingTeam().hasPlayer(thrownPlayer));
			return;
		}
		if (!state.usingSwoop) {
			publishParameter(new StepParameter(StepParameterKey.USING_SWOOP, false));
			getResult().setNextAction(StepAction.NEXT_STEP);
			return;
		}
		...
		if (state.coordinateTo == null) {
			...
			publishParameter(new StepParameter(StepParameterKey.USING_SWOOP, true));
		}
	}
	...
	@Override
	public StepSwoop initFrom(IFactorySource source, JsonValue jsonValue) {
		...
		state.thrownPlayerCoordinate = IServerJsonOption.THROWN_PLAYER_COORDINATE.getFrom(source, jsonObject);
		state.throwScatter = IServerJsonOption.THROW_SCATTER.getFrom(source, jsonObject);
