...
public class BlockInjuryEvaluator {
	private final Mode mode;
	private final boolean allowAttackerChainsaw;

	public BlockInjuryEvaluator(Mode mode, boolean allowAttackerChainsaw) {
		this.mode = mode;
		this.allowAttackerChainsaw = allowAttackerChainsaw;
	}

	public void injuryRoll(Game game, GameState gameState, DiceRoller diceRoller, Player<?> pAttacker,
		Player<?> pDefender, InjuryContext injuryContext, InjuryTypeServer<?> injuryType) {
		...
		if (mode != Mode.USE_ARMOUR_MODIFIERS_ONLY_AGAINST_TEAM_MATES && (mode == Mode.USE_MODIFIERS_AGAINST_TEAM_MATES ||
			...
			Set<InjuryModifier> injuryModifiers = factory.findInjuryModifiersWithoutNiggling(game, injuryContext, pAttacker,
				pDefender, injuryType.isStab(), injuryType.isFoul(), injuryType.isVomitLike(), injuryType.isChainsaw());
			injuryContext.addInjuryModifiers(injuryModifiers);
		}

		injuryType.setInjury(pDefender, gameState, diceRoller, injuryContext);
	}

	public void armourRoll(Game game, GameState gameState, DiceRoller diceRoller, Player<?> pAttacker,
		Player<?> pDefender, DiceInterpreter diceInterpreter, InjuryContext injuryContext, boolean roll,
		InjuryTypeServer<?> injuryType) {
		if (!injuryContext.isArmorBroken()) {
			...
			if (chainsaw != null && !skillsStackAgainstChainsaw) {
				...
				Set<ArmorModifier> armorModifiers =
					armorModifierFactory.findArmorModifiers(game, pAttacker, pDefender, injuryType.isStab(),
						injuryType.isFoul());
