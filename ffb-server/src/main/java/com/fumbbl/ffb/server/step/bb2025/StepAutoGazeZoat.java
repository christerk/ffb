...
public class StepAutoGazeZoat extends AbstractStep {

	private boolean endPlayerAction, endTurn;
	private String goToLabelOnFailure, playerId;
	private PlayerState oldPlayerState;

	public StepAutoGazeZoat(GameState pGameState) {
		super(pGameState);
	}

	@Override
	public StepId getId() {
		return StepId.AUTO_GAZE_ZOAT;
	}
				...
				case CLIENT_PLAYER_CHOICE:
					...
					if (StringTool.isProvided(clientCommandPlayerChoice.getPlayerId())) {
						...
						getResult().addReport(new ReportSkillUse(actingPlayer.getPlayerId(), 
							actingPlayer.getPlayer().getSkillWithProperty(NamedProperties.canGazeAutomaticallyThreeSquaresAway), false, SkillUse.DISTRACT_OPPONENT));
	...
	private void executeStep() {
		...
		Game game = getGameState().getGame();
		ActingPlayer actingPlayer = game.getActingPlayer();
		Skill skill = UtilCards.getUnusedSkillWithProperty(actingPlayer, NamedProperties.canGazeAutomaticallyThreeSquaresAway);
		if (skill != null) {
			...
			if (!StringTool.isProvided(playerId)) {
				...
				UtilServerDialog.showDialog(getGameState(),
					new DialogPlayerChoiceParameter(game.getActingTeam().getId(), PlayerChoiceMode.AUTO_GAZE_ZOAT, eligiblePlayers.toArray(new Player<?>[0]), null, 1), false);
	...
	private List<Player<?>> findPlayers(Game game, Player<?> player) {
		...
		return Arrays.asList(UtilPlayer.findPlayersWithTackleZones(game, game.getOtherTeam(game.getActingTeam()), playerCoordinate, 3));
	}
