...
import com.fumbbl.ffb.server.step.mixed.shared.StepTakeRoot;
import com.fumbbl.ffb.server.step.mixed.shared.StepTakeRoot.StepState;
	...
	public TakeRootBehaviour() {
		...
		registerModifier(new StepModifier<StepTakeRoot, StepState>() {
			...
			public boolean handleExecuteStepHook(StepTakeRoot step, StepState state) {
				...
				boolean startedStanding = actingPlayer.getOldPlayerState().getBase() == PlayerState.STANDING;

				if (startedStanding && !playerState.isRooted()) {
					boolean doRoll = true;
