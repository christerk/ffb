...
public final class StepIntercept extends AbstractStepWithReRoll {
	...
	private ActionStatus intercept(Player<?> pInterceptor) {
		...
		Set<InterceptionModifier> interceptionModifiers = modifierFactory.findModifiers(new InterceptionContext(game, pInterceptor, null, false));
	...
	public StepIntercept initFrom(IFactorySource source, JsonValue jsonValue) {
		...
		fInterceptorChosen = IServerJsonOption.INTERCEPTOR_CHOSEN.getFrom(source, jsonObject);
