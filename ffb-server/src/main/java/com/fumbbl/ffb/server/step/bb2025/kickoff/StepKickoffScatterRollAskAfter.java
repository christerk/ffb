...
public final class StepKickoffScatterRollAskAfter extends AbstractStep {
	...
	public StepKickoffScatterRollAskAfter(GameState pGameState) {
		super(pGameState);
	}

	public StepId getId() {
		return StepId.KICKOFF_SCATTER_ROLL_ASK_AFTER;
	}
	...
	private void executeStep() {
			...
			if (kickingPlayer != null) {
				...
				if (skillReduceKickDistance != null) {
					FieldCoordinate ballCoordinateEndWithKick = UtilServerCatchScatterThrowIn
						.findScatterCoordinate(fKickoffStartCoordinate, fScatterDirection,
							(int) Math.ceil((double) fScatterDistance / 2));
		...
		if (fUseKickChoice != null) {
			int distance = fUseKickChoice ? (int) Math.ceil((double) fScatterDistance / 2) : fScatterDistance;
			FieldCoordinate ballCoordinateEnd = UtilServerCatchScatterThrowIn.findScatterCoordinate(fKickoffStartCoordinate,
	...
	@Override
	public StepKickoffScatterRollAskAfter initFrom(IFactorySource source, JsonValue jsonValue) {
