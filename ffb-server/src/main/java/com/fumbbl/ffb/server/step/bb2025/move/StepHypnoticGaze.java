...
public class StepHypnoticGaze extends AbstractStepWithReRoll {
	...
	private void executeStep() {
		...
		if (doGaze && gazeSkill.isPresent()) {
			...
			getResult().addReport(new ReportHypnoticGazeRoll(actingPlayer.getPlayerId(), successful,
				roll, minimumRoll, reRolled, new GazeModifier[0], game.getDefenderId()));
			if (successful) {
				PlayerState oldVictimState = game.getFieldModel().getPlayerState(game.getDefender());
				if (!oldVictimState.isConfused()) {
					game.getFieldModel().setPlayerState(game.getDefender(), oldVictimState.changeConfused(true));
				}
	...
	@Override
	public StepHypnoticGaze initFrom(IFactorySource source, JsonValue jsonValue) {
