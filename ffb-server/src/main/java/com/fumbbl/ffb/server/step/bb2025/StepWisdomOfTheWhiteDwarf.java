...
public class StepWisdomOfTheWhiteDwarf extends AbstractStep {
	...
	private void executeStep() {
		...
		Player<?> target = game.getPlayerById(playerId);

		SkillFactory skillFactory = game.getFactory(FactoryType.Factory.SKILL);
		Set<Skill> targetOwnedSkills = target.getSkillsIncludingTemporaryOnes();

		List<SkillWithValue> gainAbleSkills = Constant.getGrantAbleSkills(skillFactory).stream()
			.filter(swv -> !targetOwnedSkills.contains(swv.getSkill()))
			.sorted(Comparator.comparing(o -> o.getSkill().getName()))
			.collect(Collectors.toList());

		if (!ArrayTool.isProvided(gainAbleSkills.toArray())) {
			getResult().setNextAction(StepAction.NEXT_STEP);
			return;
		}
		...
		if (skill == null) {
			...
			getResult().addReport(new ReportSkillUse(actingPlayer.getPlayerId(), grantingSkill, true,
			...
			if (gainAbleSkills.size() == 1) {
				...
				UtilServerDialog.showDialog(getGameState(),
					new DialogSelectSkillParameter(target.getId(),
		...
		game.getFieldModel().addWisdomSkill(target.getId(), gainedSkill);
		getResult().addReport(new ReportPlayerEvent(target.getId(), "gains " + gainedSkill.getSkill().getName()));
