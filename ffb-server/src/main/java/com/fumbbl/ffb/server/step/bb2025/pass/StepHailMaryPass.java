...
public final class StepHailMaryPass extends AbstractStepWithReRoll {
	...
	public StepCommandStatus handleCommand(ReceivedCommand pReceivedCommand) {
		...
		if (commandStatus == StepCommandStatus.UNHANDLED_COMMAND && pReceivedCommand.getId() == NetCommandId.CLIENT_USE_SKILL) {
			...
			} else if (commandUseSkill.getSkill().hasSkillProperty(NamedProperties.dontDropFumbles)) {
				state.usingSafePass = commandUseSkill.isSkillUsed();
				commandStatus = StepCommandStatus.EXECUTE_STEP;
			} else {
	...
	public JsonObject toJsonValue() {
		...
		IServerJsonOption.USING_SAFE_PASS.addTo(jsonObject, state.usingSafePass);
		return jsonObject;
	}
	...
	public StepHailMaryPass initFrom(IFactorySource source, JsonValue jsonValue) {
		...
		state.usingSafePass = IServerJsonOption.USING_SAFE_PASS.getFrom(source, jsonObject);
		return this;
	}
	...
	public static class StepState {
		...
		public Boolean usingModifyingSkill, usingSafePass;
