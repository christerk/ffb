...
public class StepBlockChoice extends AbstractStep {
	...
	private void executeStep() {
		...
		switch (fBlockResult) {
			...
			case BOTH_DOWN:
				...
				break;
			case POW_PUSHBACK:
				boolean usesWatchOut = false;
				boolean blockedByOpponent = actingPlayer.getPlayer().getTeam() != game.getDefender().getTeam();
				Skill defenderDodgeSkill = game.getDefender().getSkillWithProperty(NamedProperties.ignoreDefenderStumblesResult);
				if (defenderDodgeSkill == null && blockedByOpponent) {
					Optional<Skill> unusedSKill = UtilCards.getUnusedSkillWithProperty(game.getDefender(), NamedProperties.ignoresDefenderStumblesResultForFirstBlock);

					if (unusedSKill.isPresent()) {
						defenderDodgeSkill = unusedSKill.get();
						usesWatchOut = true;
					}
				}
	...
	@Override
	public StepBlockChoice initFrom(IFactorySource source, JsonValue jsonValue) {
