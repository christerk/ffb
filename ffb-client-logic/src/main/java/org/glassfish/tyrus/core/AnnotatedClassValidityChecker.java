...
/**
 * Used when processing a class annotated with {@link @ServerEndpoint} to check
 * that it complies with specification.
 *
...
class AnnotatedClassValidityChecker {
	...
	/**
	 * Checks whether the params of the method annotated with
	 * {@link javax.websocket.OnMessage} comply with the specification.
	 * <p>
	 * Voluntary parameters of type {@link javax.websocket.Session} and parameters
	 * annotated with {@link javax.websocket.server.PathParam} are checked in
	 * advance in {@link AnnotatedEndpoint}.
	 */
	public void checkOnMessageParams(Method method, MessageHandler handler) {
		try {
			handlerManager.addMessageHandler(handler);
		} catch (IllegalStateException ise) {
			collector.addException(new DeploymentException(LocalizationMessages.CLASS_CHECKER_ADD_MESSAGE_HANDLER_ERROR(
	...
	/**
	 * Checks whether the params of method annotated with
	 * {@link javax.websocket.OnOpen} comply with the specification.
	 * <p>
	 * Voluntary parameters of type {@link javax.websocket.Session} and parameters
	 * annotated with {@link javax.websocket.server.PathParam} are checked in
	 * advance in {@link AnnotatedEndpoint}.
	...
	public void checkOnOpenParams(Method method, Map<Integer, Class<?>> params) {
		for (Class<?> value : params.values()) {
			if (value != EndpointConfig.class) {
				logDeploymentException(new DeploymentException(LocalizationMessages
						.CLASS_CHECKER_FORBIDDEN_WEB_SOCKET_OPEN_PARAM(annotatedClass.getName(), method.getName(), value)));
			}
		}
	}
	...
	/**
	 * Checks whether the params of method annotated with
	 * {@link javax.websocket.OnClose} comply with the specification.
	 *
	...
	public void checkOnCloseParams(Method method, Map<Integer, Class<?>> params) {
		for (Class<?> value : params.values()) {
			if (value != CloseReason.class) {
				logDeploymentException(new DeploymentException(LocalizationMessages
						.CLASS_CHECKER_FORBIDDEN_WEB_SOCKET_CLOSE_PARAM(annotatedClass.getName(), method.getName())));
			}
		}
	}
	...
	/**
	 * Checks whether the params of method annotated with
	 * {@link javax.websocket.OnError} comply with the specification.
	 *
	 * @param params unknown params of the method.
	 */
	public void checkOnErrorParams(Method method, Map<Integer, Class<?>> params) {
		...
		for (Class<?> value : params.values()) {
			if (value != Throwable.class) {
				logDeploymentException(new DeploymentException(LocalizationMessages
						.CLASS_CHECKER_FORBIDDEN_WEB_SOCKET_ERROR_PARAM(annotatedClass.getName(), method.getName(), value)));
			} else {
				if (throwablePresent) {
					logDeploymentException(new DeploymentException(LocalizationMessages
							.CLASS_CHECKER_MULTIPLE_IDENTICAL_PARAMS(annotatedClass.getName(), method.getName())));
				}
		...
		if (!throwablePresent) {
			logDeploymentException(new DeploymentException(
					LocalizationMessages.CLASS_CHECKER_MANDATORY_PARAM_MISSING(annotatedClass.getName(), method.getName())));
		}
	}
