...
public class UriComponent {
	...
	public enum Type {
		...
		QUERY,
		/**
		 * The URI query component type that is a query parameter, space character is
		 * encoded as {@code +}.
		 */
		QUERY_PARAM,
		/**
	...
	/**
	 * Validates the legal characters of a percent-encoded string that represents a
	 * URI component type.
	 *
	 ...
	 * UTF-8 encoding. Percent-encoded characters will be recognized and not double
	 * encoded.
	 *
	 * @param s the string to be encoded.
	 * @param t the URI component type identifying the ASCII characters that must be
	 *          percent-encoded.
	...
	/**
	 * Encodes the characters of string that are either non-ASCII characters or are
	 * ASCII characters that must be percent-encoded using the UTF-8 encoding.
	 *
	 * @param s the string to be encoded.
	 * @param t the URI component type identifying the ASCII characters that must be
	 *          percent-encoded.
	...
	/**
	 * Decodes characters of a string that are percent-encoded octets using UTF-8
	 * decoding (if needed).
	 * <p>
	 * It is assumed that the string is valid according to an (unspecified) URI
	 * component type. If a sequence of contiguous percent-encoded octets is not a
	 * valid UTF-8 character then the octets are replaced with '\uFFFD'.
	 * <p>
	 * If the URI component is of type HOST then any "%" found between "[]" is left
	 * alone. It is an IPv6 literal with a scope_id.
	 * <p>
	 ...
	 * query parameter values can be controlled using the {@code decode} parameter
	 * flag.
	 *
	 * @param u      the URI.
	 * @param decode {@code true} if the returned query parameter values of the
	 ...
	 *
	 * @param q            the query component in encoded form.
	 * @param decodeNames  {@code true} if the returned query parameter names of the
	 *                     query component should be in decoded form.
	 * @param decodeValues {@code true} if the returned query parameter values of
	 *                     the query component should be in decoded form.
	 * @return the multivalued map of query parameters.
	 */
	 ...
	 * @param decode true if the path segments of the path component should be in
	 *               decoded form.
	 ...
	 * Decode the path component of a URI as path segments.
	 * <p>
	 * Any '/' character in the path is considered to be a deliminator between two
	 * path segments. Thus if the path is '/' then the path segment list will
	 * contain two empty path segments. If the path is "//" then the path segment
	 * list will contain three empty path segments. If the path is "/a/" the path
	 * segment list will consist of the following path segments in order: "", "a"
	 * and "".
	 *
	...
	public static void decodePathSegment(final List<PathSegment> segments, final String segment, final boolean decode) {
		...
		if (colon != -1) {
			segments.add(
					new PathSegmentImpl((colon == 0) ? "" : segment.substring(0, colon), decode, decodeMatrix(segment, decode)));
		} else {
	 ...
	 * @param decode      true if the matrix parameters of the path segment
	 *                    component should be in decoded form.
	...
	/**
	 * Return the {@code Request-Uri} representation as defined by HTTP spec. For
	 * example:
	 * 
	 * <pre>
	 ...
	 * @return {@code Request-Uri} representation or {@code null} if {@code uri} is
	 *         not provided.
	 */
