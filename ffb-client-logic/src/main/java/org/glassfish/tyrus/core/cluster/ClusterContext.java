 ...
 * <p>
 * There is exactly one instance per cluster node and all communication is
 * realized using this instance.
 *
...
public abstract class ClusterContext {
	 ...
	 * @return future representing the send event.
	 *         {@link java.util.concurrent.Future#get()} returns when there is an
	 *         acknowledge from the other node that the message has been
	 *         successfully sent. If there is any exception, it will be wrapped into
	 ...
	 *                    {@link javax.websocket.SendHandler#onResult(javax.websocket.SendResult)}
	 *                    will be invoked.
	 ...
	 *
	 * @param endpointPath endpoint path identifying sessions alignment to the
	 *                     endpoint.
	 ...
	 *
	 * @param sessionId remote session id.
	 * @return future representing the event.
	 *         {@link java.util.concurrent.Future#get()} returns when there is an
	 *         acknowledge from the other node that the command was successfully
	 *         executed. If there is any exception, it will be wrapped into
	 *         {@link java.util.concurrent.ExecutionException} and thrown.
	 */
	...
	/**
	 * Get set containing session ids of all remote sessions registered to given
	 * endpoint path.
	 *
	 ...
	 * <p>
	 * Session id will be broadcasted to other nodes which will call
	 * {@link #getDistributedSessionProperties(String)} and process its values. The
	 * map must be ready before this method is invoked.
	 *
	 * @param sessionId    session id to be registered.
	 * @param endpointPath endpoint path identifying sessions alignment to the
	 *                     endpoint.
	 * @param listener     session event listener. When remote node sends a message
	 *                     to this session, it will be invoked.
	 ...
	 * <p>
	 * Gets notification about session creation
	 * {@link org.glassfish.tyrus.core.cluster .SessionListener#onSessionOpened(String)}
	 * and destruction
	 * {@link org.glassfish.tyrus.core.cluster .SessionListener#onSessionClosed(String)}.
	 *
	 * @param endpointPath endpoint path identifying sessions alignment to the
	 *                     endpoint.
	 ...
	 * <p>
	 * Gets notification about broadcasted messages. Used as an optimized variant of
	 * standard websocket broadcast pattern. In this case, only one message is sent
	 * to all cluster nodes (instead {@code n} when {@code n} represent number of
	 ...
	 * Changes must be propagated to remote instances.
	 *
	 * @param connectionId connection id. Connection id may be shared among
	 *                     subsequent TCP connection - represents logical
	 *                     connection.
	 * @return distributed map containing session properties.
	 */
	 ...
	 * <p>
	 * This will stop whole clustered node, any operation related to this cluster
	 * context will fail after this method is invoked.
	 */
