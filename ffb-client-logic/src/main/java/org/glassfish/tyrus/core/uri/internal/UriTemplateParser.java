...
public class UriTemplateParser {
	 ...
	 * @throws IllegalArgumentException if the template is null, an empty string or
	 *                                  does not conform to a JAX-RS URI template.
	 */
	...
	private void parse(final CharacterIterator ci) {
		try {
			...
			throw new IllegalArgumentException(
					"Invalid syntax for the template, \"" + template + "\". Check if a path parameter is terminated with a '}'.",
					ex);
		}
	}
...
//                groupCounts.add(1);
				...
				if (nameToPattern.containsKey(name)) {
					if (!nameToPattern.get(name).equals(namePattern)) {
						throw new IllegalArgumentException(
								"The name '" + name + "' is declared more than once with different regular expressions");
					}
				} else {
			...
			throw new IllegalArgumentException(
					"Invalid syntax for the expression '" + nameRegexString + "' associated with the name '" + name + "'", ex);
		}
