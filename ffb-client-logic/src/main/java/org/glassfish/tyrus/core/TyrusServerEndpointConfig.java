...
/**
 * Configuration {@link javax.websocket.server.ServerEndpointConfig} enhanced to
 * offer tyrus specific attributes like maxSessions. Declarative way to define
 * maxSessions is also available using annotation {@link MaxSessions}.
 *
...
public interface TyrusServerEndpointConfig extends ServerEndpointConfig {
	 ...
	 * "/foo").build();}
	 * <p>
	 * Building a configuration with no subprotocols, limited number of sessions
	 * (100) and a custom configurator.
	 * 
	 * <pre>
	 * <code>
	...
	public final class Builder {
		...
		public TyrusServerEndpointConfig build() {
			...
			final ServerEndpointConfig serverEndpointConfig = ServerEndpointConfig.Builder.create(endpointClass, path)
					.subprotocols(subprotocols).extensions(extensions).encoders(encoders).decoders(decoders)
