...
public class AnimationProjector {
	...
	public double findScale(AnimationType animationType) {
		if (xAxisAnimation) {
			return findScale(((currentWidth - startDimension.width) / (double) (endDimension.width - startDimension.width)) * 2, animationType);
		} else {
			return findScale(((currentHeight - startDimension.height) / (double) (endDimension.height - startDimension.height)) * 2, animationType);
		}
	}
