...
public class ReplayControl extends JPanel implements MouseInputListener {
  ...
  private class ReplayButton {
    ...
    public boolean isMouseOver(MouseEvent pMouseEvent) {
      return ((pMouseEvent.getX() >= (fPosition.x - (_ICON_GAP / 2))) && (pMouseEvent.getX() < (fPosition.x + _ICON_WIDTH + (_ICON_GAP / 2))));
    }
  ...
  public ReplayControl(FantasyFootballClient pClient) {
    ...
    fButtonPause = new ReplayButton(
      new Point((WIDTH / 2) - (_ICON_WIDTH / 2), 1),
      IIconProperty.REPLAY_PAUSE, IIconProperty.REPLAY_PAUSE_ACTIVE, IIconProperty.REPLAY_PAUSE_SELECTED
    );
    fButtonPlayBackward = new ReplayButton(
      new Point(fButtonPause.getPosition().x - _ICON_WIDTH - _ICON_GAP, 1),
      IIconProperty.REPLAY_PLAY_BACKWARD, IIconProperty.REPLAY_PLAY_BACKWARD_ACTIVE, IIconProperty.REPLAY_PLAY_BACKWARD_SELECTED
    );
    fButtonFastBackward = new ReplayButton(
      new Point(fButtonPlayBackward.getPosition().x - _ICON_WIDTH - _ICON_GAP, 1),
      IIconProperty.REPLAY_FAST_BACKWARD, IIconProperty.REPLAY_FAST_BACKWARD_ACTIVE, IIconProperty.REPLAY_FAST_BACKWARD_SELECTED
    );
    fButtonSkipBackward = new ReplayButton(
      new Point(fButtonFastBackward.getPosition().x - _ICON_WIDTH - _ICON_GAP, 1),
      IIconProperty.REPLAY_SKIP_BACKWARD, IIconProperty.REPLAY_SKIP_BACKWARD_ACTIVE, IIconProperty.REPLAY_SKIP_BACKWARD_SELECTED
    );
    fButtonPlayForward = new ReplayButton(
      new Point(fButtonPause.getPosition().x + _ICON_WIDTH + _ICON_GAP, 1),
      IIconProperty.REPLAY_PLAY_FORWARD, IIconProperty.REPLAY_PLAY_FORWARD_ACTIVE, IIconProperty.REPLAY_PLAY_FORWARD_SELECTED
    );
    fButtonFastForward = new ReplayButton(
      new Point(fButtonPlayForward.getPosition().x + _ICON_WIDTH + _ICON_GAP, 1),
      IIconProperty.REPLAY_FAST_FORWARD, IIconProperty.REPLAY_FAST_FORWARD_ACTIVE, IIconProperty.REPLAY_FAST_FORWARD_SELECTED
    );
    fButtonSkipForward = new ReplayButton(
      new Point(fButtonFastForward.getPosition().x + _ICON_WIDTH + _ICON_GAP, 1),
      IIconProperty.REPLAY_SKIP_FORWARD, IIconProperty.REPLAY_SKIP_FORWARD_ACTIVE, IIconProperty.REPLAY_SKIP_FORWARD_SELECTED
    );
  ...
  private void refresh() {
    ...
    if (replayer.isRunning()) {
      ...
      String speed = new StringBuilder().append((replayer.getReplaySpeed() > 0) ? replayer.getReplaySpeed() : "0.5").append("x").toString();
