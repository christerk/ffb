...
public class TurnTimerTask extends TimerTask {
  ...
  public void run() {
    ...
    if ((ClientMode.PLAYER == getClient().getMode()) && game.isTurnTimeEnabled() && !clientData.isTurnTimerStopped() && !game.isHomePlaying() && !game.isTimeoutPossible() && !game.isTimeoutEnforced() && (game.getOptions().getOptionValue(GameOption.TURNTIME).getValue() > 0) && (game.getTurnTime() >= game.getOptions().getOptionValue(GameOption.TURNTIME).getValue() * 1000)) {
