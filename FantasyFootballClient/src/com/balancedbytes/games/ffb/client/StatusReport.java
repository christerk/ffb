...
public class StatusReport {
	...
	/*
	 * public void reportSpirallingExpenses(ReportSpirallingExpenses pReport) { Game
	 * game = getClient().getGame(); if ((pReport.getExpensesHomeTeam() > 0) ||
	 * (pReport.getExpensesAwayTeam() > 0)) { println(getIndent(), TextStyle.ROLL,
	 * "Spiralling Expenses"); if (pReport.getExpensesHomeTeam() > 0) {
	 * print(getIndent() + 1, TextStyle.HOME, game.getTeamHome().getName());
	 * StringBuilder status = new StringBuilder();
	 * status.append(" must pay ").append
	 * (pReport.getExpensesHomeTeam()).append(" in expenses."); println(getIndent()
	 * + 1, TextStyle.NONE, status.toString()); } if (pReport.getExpensesAwayTeam()
	 * > 0) { print(getIndent() + 1, TextStyle.AWAY, game.getTeamAway().getName());
	 * StringBuilder status = new StringBuilder();
