    ...
    if (dialogParameter != null) {
      ...
      if ((ClientMode.PLAYER == getClient().getMode()) && game.getTeamHome().getId().equals(dialogParameter.getTeamId())) {
        ...
        if (ArrayTool.isProvided(dialogParameter.getPlayerIds()) && dialogParameter.getPlayerIds().length == 1) {
          ...
          setDialog(new DialogPlayerChoice(getClient(), "Select players to argue the call for", dialogParameter.getPlayerIds(), null, dialogParameter.getPlayerIds().length, null));
        }
