...
public class DialogLogin extends Dialog {
  ...
  public DialogLogin(
  	FantasyFootballClient pClient,
  	byte[] pEncodedPassword,
  	int pPasswordLength,
  	String pTeamHomeName,
  	String pTeamAwayName,
  	boolean pShowGameName
  ) {
    ...
    fFieldPassword.addActionListener(
      new ActionListener() {
    ...
    fButtonCreate.addActionListener(
      new ActionListener() {
        public void actionPerformed(ActionEvent pActionEvent) {
          fListGames = false;
          checkAndCloseDialog();
  ...
  private void checkAndCloseDialog() {
  	if (StringTool.isProvided(getCoach()) && ((fPasswordLength < 0) || ArrayTool.isProvided(fFieldPassword.getPassword()))) {
