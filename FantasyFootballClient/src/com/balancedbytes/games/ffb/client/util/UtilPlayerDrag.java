...
public class UtilPlayerDrag {
  ...
  private static FieldCoordinate getFieldFieldCoordinate(FantasyFootballClient pClient, int pMouseX, int pMouseY) {
    if ((pMouseX >= 0) && (pMouseX < FieldLayer.FIELD_IMAGE_WIDTH) && (pMouseY >= 0) && (pMouseY < FieldLayer.FIELD_IMAGE_HEIGHT)) {
  ...
  private static FieldCoordinate getBoxFieldCoordinate(FantasyFootballClient pClient, int pMouseX, int pMouseY) {
    if ((pMouseX >= 0) && (pMouseX < BoxComponent.WIDTH) && (pMouseY >= 0) && (pMouseY < BoxComponent.HEIGHT)) {
      ...
      int y = (((pMouseY - boxTitleOffset) / BoxComponent.FIELD_SQUARE_SIZE) * 3) + (pMouseX / BoxComponent.FIELD_SQUARE_SIZE);
  ...
  private static void initPlayerDragging(FantasyFootballClient pClient, FieldCoordinate pCoordinate, boolean pBoxMode) {
    ...
    boolean initDragAllowed = ((ClientMode.PLAYER == pClient.getMode()) && (player != null) && game.getTeamHome().hasPlayer(player) && pClient.getClientState().isInitDragAllowed(pCoordinate));
    if (initDragAllowed) {
      if (pBoxMode) {
        initDragAllowed = (((playerState.getBase() == PlayerState.STANDING) && playerState.isActive()) || (playerState.getBase() == PlayerState.RESERVE) || (playerState.getBase() == PlayerState.BEING_DRAGGED));
      } else {
        initDragAllowed = (((playerState.getBase() == PlayerState.STANDING) && playerState.isActive()) || (playerState.getBase() == PlayerState.BEING_DRAGGED));
      }
    }
    if (initDragAllowed) {
      if (player != null) {
      ...
      if (playerState.getBase() == PlayerState.BEING_DRAGGED) {
        ...
        if (playerCoordinate != null) {
          if (playerCoordinate.isBoxCoordinate()) {
            ...
            game.getFieldModel().setPlayerState(player, playerState.changeBase(PlayerState.STANDING).changeActive(true));
          }
        }
      }
    }
