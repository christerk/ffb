  ...
  protected void clickOnPlayer(Player pPlayer) {
    ...
    if (pPlayer == actingPlayer.getPlayer()) {
      ...
      if (game.getDefender() != null) {
        ...
        getClient().getCommunication().sendThrowTeamMate(actingPlayer.getPlayerId(), game.getFieldModel().getPlayerCoordinate(pPlayer));
      }
    }
  }
...
//    if ((PlayerAction.THROW_TEAM_MATE == actingPlayer.getPlayerAction()) && (game.getPassCoordinate() == null)) {
  ...
  private boolean canBeThrown(Player pPlayer) {
    ...
    return ( UtilCards.hasSkillWithProperty(actingPlayer.getPlayer(), NamedProperties.canThrowTeamMates) && UtilCards.hasSkillWithProperty(pPlayer, NamedProperties.canBeThrown) && catcherState.hasTacklezones() && catcherCoordinate.isAdjacent(throwerCoordinate) && (actingPlayer.getPlayer().getTeam() == pPlayer.getTeam()));
  }
  ...
  private void markThrowablePlayers() {
    ...
    if ((game.getDefender() == null) && ArrayTool.isProvided(throwablePlayers)) {
      userInterface.getFieldComponent().getLayerRangeRuler().markPlayers(throwablePlayers, FieldLayerRangeRuler.COLOR_THROWABLE_PLAYER);
    } else {
