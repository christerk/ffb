...
public class ClientStateSelect extends ClientState {
  ...
  private void createAndShowPopupMenuForPlayer(Player pPlayer) {
    ...
    if (isBlockActionAvailable(pPlayer)) {
      JMenuItem blockAction = new JMenuItem("Block Action", new ImageIcon(iconCache.getIconByProperty(IIconProperty.ACTION_BLOCK)));
    ...
    if (isMultiBlockActionAvailable(pPlayer)) {
      JMenuItem multiBlockAction = new JMenuItem("Multiple Block", new ImageIcon(iconCache.getIconByProperty(IIconProperty.ACTION_MUTIPLE_BLOCK)));
    ...
    if (isThrowBombActionAvailable(pPlayer)) {
      JMenuItem moveAction = new JMenuItem("Throw Bomb Action", new ImageIcon(iconCache.getIconByProperty(IIconProperty.ACTION_BOMB)));
    ...
    if (isBlitzActionAvailable(pPlayer)) {
      JMenuItem blitzAction = new JMenuItem("Blitz Action", new ImageIcon(iconCache.getIconByProperty(IIconProperty.ACTION_BLITZ)));
    ...
    if (isFoulActionAvailable(pPlayer)) {
      JMenuItem foulAction = new JMenuItem("Foul Action", new ImageIcon(iconCache.getIconByProperty(IIconProperty.ACTION_FOUL)));
    ...
    if (isPassActionAvailable(pPlayer)) {
      JMenuItem passAction = new JMenuItem("Pass Action", new ImageIcon(iconCache.getIconByProperty(IIconProperty.ACTION_PASS)));
    ...
    if (isHandOverActionAvailable(pPlayer)) {
      JMenuItem handOverAction = new JMenuItem("Hand Over Action", new ImageIcon(iconCache.getIconByProperty(IIconProperty.ACTION_HAND_OVER)));
    ...
    if (isThrowTeamMateActionAvailable(pPlayer)) {
      JMenuItem throwTeamMateAction = new JMenuItem("Throw Team-Mate Action", new ImageIcon(iconCache.getIconByProperty(IIconProperty.ACTION_PASS)));
    ...
    if (isRecoverFromGazeActionAvailable(pPlayer)) {
      JMenuItem confusionAction = new JMenuItem("Recover from Gaze & End Move", new ImageIcon(iconCache.getIconByProperty(IIconProperty.ACTION_STAND_UP)));
      confusionAction.setMnemonic(IPlayerPopupMenuKeys.KEY_RECOVER);
      confusionAction.setAccelerator(KeyStroke.getKeyStroke(IPlayerPopupMenuKeys.KEY_RECOVER, 0));
      menuItemList.add(confusionAction);
    }
    ...
    if (isStandUpActionAvailable(pPlayer)) {
      JMenuItem standUpAction = new JMenuItem("Stand Up & End Move", new ImageIcon(iconCache.getIconByProperty(IIconProperty.ACTION_STAND_UP)));
