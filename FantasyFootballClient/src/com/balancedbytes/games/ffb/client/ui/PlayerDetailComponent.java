...
//      }
    ...
    if (fPlayer != null) {
      ...
      if (statModifier > 0) {
        ...
        if (statModifier > 1) {
          ...
          pG2d.fillPolygon(new int[] { pX + 2, pX + 2, pX + _STAT_BOX_WIDTH - 3 }, new int[] { pY + _STAT_BOX_HEIGHT - _STAT_BOX_INNER_HEIGHT - 2, pY + _STAT_BOX_HEIGHT - 2, pY + _STAT_BOX_HEIGHT - 2 }, 3);
      ...
      if (statModifier < 0) {
        ...
        if (statModifier < -1) {
          ...
          pG2d.fillPolygon(new int[] { pX + 2, pX + _STAT_BOX_WIDTH - 3, pX + _STAT_BOX_WIDTH - 3 }, new int[] { pY + _STAT_BOX_HEIGHT - _STAT_BOX_INNER_HEIGHT - 2, pY + _STAT_BOX_HEIGHT - _STAT_BOX_INNER_HEIGHT - 2, pY + _STAT_BOX_HEIGHT - 2 }, 3);
          // pG2d.fillRect(pX + _STAT_BOX_WIDTH - 7, pY + _STAT_BOX_HEIGHT - _STAT_BOX_INNER_HEIGHT - 1, 4, 4);
        }
      }
      Color statColor = pStatIsRed ? Color.RED : Color.BLACK;
      drawCenteredText(pG2d, pX + (_STAT_BOX_WIDTH / 2), pY + _STAT_BOX_HEIGHT - 4, statColor, Integer.toString(pValue));
    }
  }
  ...
  private int findDisplayMode() {
    ...
    if (!getSideBar().isBoxOpen()) {
      ...
      SideBarComponent otherSideBar = getSideBar().isHomeSide() ? userInterface.getSideBarAway() : userInterface.getSideBarHome();
