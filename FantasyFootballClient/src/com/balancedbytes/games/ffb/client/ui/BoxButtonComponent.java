...
public class BoxButtonComponent extends JPanel implements MouseListener, MouseMotionListener {
  ...
  public BoxButtonComponent(SideBarComponent pSideBar) {
    ...
    if (getSideBar().isHomeSide()) {
      ...
      fButtonLocations.put(BoxType.OUT, new Rectangle((WIDTH / 2) + 1, 0, _BUTTON_DIMENSION.width, _BUTTON_DIMENSION.height));
    } else {
      fButtonLocations.put(BoxType.OUT, new Rectangle(1, 0, _BUTTON_DIMENSION.width, _BUTTON_DIMENSION.height));
      fButtonLocations.put(BoxType.RESERVES, new Rectangle((WIDTH / 2) + 1, 0, _BUTTON_DIMENSION.width, _BUTTON_DIMENSION.height));
    }
    setLayout(null);
  ...
  private void drawButton(BoxType pBox) {
    if (pBox != null) {
      ...
      String buttonText = new StringBuilder().append(countBoxElements(pBox, getSideBar().isHomeSide())).append(" ").append(pBox.getShortcut()).toString();
  ...
  public int countBoxElements(BoxType pBox, boolean pHomeSide) {
    ...
    if (BoxType.OUT == pBox) {
      ...
      // elements += countBoxElements(pHomeSide ? FieldCoordinate.MNG_HOME_X: FieldCoordinate.MNG_AWAY_X);
    }
    return elements;
  }
