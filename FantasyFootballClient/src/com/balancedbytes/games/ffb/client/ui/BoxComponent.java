...
public class BoxComponent extends JPanel implements MouseListener, MouseMotionListener {
  ...
  private void drawPlayers() {
    ...
    if (BoxType.RESERVES == fOpenBox) {
      yPosition = drawPlayersInBox(getSideBar().isHomeSide() ? FieldCoordinate.RSV_HOME_X : FieldCoordinate.RSV_AWAY_X, yPosition);
    }
    if (BoxType.OUT == fOpenBox) {
      yPosition = drawPlayersInBox(getSideBar().isHomeSide() ? FieldCoordinate.KO_HOME_X : FieldCoordinate.KO_AWAY_X, yPosition);
      yPosition = drawPlayersInBox(getSideBar().isHomeSide() ? FieldCoordinate.BH_HOME_X : FieldCoordinate.BH_AWAY_X, yPosition);
      yPosition = drawPlayersInBox(getSideBar().isHomeSide() ? FieldCoordinate.SI_HOME_X : FieldCoordinate.SI_AWAY_X, yPosition);
      yPosition = drawPlayersInBox(getSideBar().isHomeSide() ? FieldCoordinate.RIP_HOME_X : FieldCoordinate.RIP_AWAY_X, yPosition);
      yPosition = drawPlayersInBox(getSideBar().isHomeSide() ? FieldCoordinate.BAN_HOME_X : FieldCoordinate.BAN_AWAY_X, yPosition);
      // yPosition = drawPlayersInBox(getSideBar().isHomeSide() ? FieldCoordinate.MNG_HOME_X : FieldCoordinate.MNG_AWAY_X, yPosition);
    }
  }
  ...
  private int drawPlayersInBox(int pXCoordinate, int pYPosition) {
    ...
    for (int y = 0; y < MAX_BOX_ELEMENTS; y++) {
      ...
      if ((player != null) || (pXCoordinate == FieldCoordinate.RSV_HOME_X))  {
        ...
        BoxSlot boxSlot = new BoxSlot(new Rectangle(locationX, locationY, FIELD_SQUARE_SIZE, FIELD_SQUARE_SIZE), boxState);
