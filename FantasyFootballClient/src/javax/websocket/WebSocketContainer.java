...
public interface WebSocketContainer {
	 ...
	 * to send a websocket message for all RemoteEndpoints associated with this
	 * container. A non-positive number indicates the implementation will not
	 ...
	 * send a websocket message for all RemoteEndpoints associated with this
	 * container. A non-positive number indicates the implementation will not
	 * timeout attempting to send a websocket message asynchronously. Note this
	 * default may be overridden in each RemoteEndpoint.
	 */
	 ...
	 * {@link javax.websocket.server.ServerEndpoint} annotation. This method blocks
	 ...
	 * endpoint, services like dependency injection that are supported, for example,
	 * when the implementation is part of the Java EE platform may not be available.
	 * If the client endpoint uses dependency injection, use
	 * {@link WebSocketContainer#connectToServer(java.lang.Class, java.net.URI)}
	 * instead.
	 *
	 ...
	 *                               prevented the client endpoint being connected
	 *                               to its server.
	 * @throws IllegalStateException if called during the deployment phase of the
	 *                               containing application.
	 */
	Session connectToServer(Object annotatedEndpointInstance, URI path) throws DeploymentException, IOException;

	/**
	 * Connect the supplied annotated endpoint to its server. The supplied object
	...
	/**
	 * Connect the supplied programmatic client endpoint instance to its server with
	 * the given configuration. This method blocks until the connection is
	 * established, or throws an error if the connection could not be made. If the
	 * developer uses this method to deploy the client endpoint, services like
	 * dependency injection that are supported, for example, when the implementation
	 * is part of the Java EE platform may not be available. If the client endpoint
	 ...
	 * in this container will be closed if it has been inactive. A value that is 0
	 * or negative indicates the sessions will never timeout due to inactivity. The
	 * value may be overridden on a per session basis using
	...
	/**
	 * Sets the default time in milliseconds after which any web socket sessions in
	 * this container will be closed if it has been inactive. A value that is 0 or
	 * negative indicates the sessions will never timeout due to inactivity. The
	 * value may be overridden on a per session basis using
	 * {@link Session#setMaxIdleTimeout(long) }
	 *
	 ...
	 * container will buffer. This default may be overridden on a per session basis
	 * using {@link Session#setMaxBinaryMessageBufferSize(int) }
	 *
