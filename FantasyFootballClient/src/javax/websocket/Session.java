 ...
 * Calling any of its methods (with the exception of the close() methods) once
 * the session has been closed will result in an
 * {@link java.lang.IllegalStateException} being thrown. Developers should
 * retrieve any information from the session during the {@link Endpoint#onClose}
 * method. Following the convention of {@link java.io.Closeable} calling the
 * Session close() methods after the Session has been closed has no effect.
 * <p>
...
public interface Session extends Closeable {
	...
	/**
	 * Register to handle to incoming messages in this conversation. A maximum of
	 * one message handler per native websocket message type (text, binary, pong)
	 * may be added to each Session. I.e. a maximum of one message handler to handle
	 * incoming text messages a maximum of one message handler for handling incoming
	 * binary messages, and a maximum of one for handling incoming pong messages.
	 * For further details of which message handlers handle which of the native
	 * websocket message types please see {@link MessageHandler.Whole} and
	 * {@link MessageHandler.Partial}. Adding more than one of any one type will
	 * result in a runtime exception.
	 * <p>
	 * This method is not safe to use unless you are providing an anonymous class
	 * derived directly from {@link javax.websocket.MessageHandler.Whole} or
	 * {@link javax.websocket.MessageHandler.Partial}. In all other cases (Lambda
	 * Expressions, more complex inheritance or generic type arrangements), one of
	 * the following methods have to be used:
	 ...
	 * @throws IllegalStateException if there is already a MessageHandler registered
	 *                               for the same native websocket message type as
	 *                               this handler.
	 */
	 ...
	 * result in a runtime exception.
	 *
	 * @param clazz   type of the message processed by message handler to be
	 *                registered.
	...
	/**
	 * Remove the given MessageHandler from the set belonging to this session. This
	 * method may block if the given handler is processing a message until it is no
	 * longer in use.
	 *
	 ...
	 * taken as the value of the Sec-WebSocket-Version header used in the opening
	 * handshake. i.e. "13".
	 *
	...
	/**
	 * Return the number of milliseconds before this conversation may be closed by
	 * the container if it is inactive, i.e. no messages are either sent or received
	 * in that time.
	 *
	...
	/**
	 * Set the non-zero number of milliseconds before this session will be closed by
	 * the container if it is inactive, ie no messages are either sent or received.
	 * A value that is 0 or negative indicates the session will never timeout due to
	 * inactivity.
	 *
	 ...
	 * If the implementation receives a binary message that it cannot buffer because
	 * it is too large, it must close the session with a close code of
	 ...
	 * is too large, it must close the session with a close code of
	 * {@link CloseReason.CloseCodes#TOO_BIG}.
	 *
	 ...
	 * The identifier is assigned by the web socket implementation and is
	 ...
	 * If the application cannot determine a suitable close code to use for the
	 * closeReason, it is recommended to use
	...
	/**
	 * Return the request parameters associated with the request this session was
	 * opened under.
	 *
	...
	/**
	 * Return the query string associated with the request this session was opened
	 * under.
	 *
	 ...
	 *
	 * @return the unmodifiable map of path parameters. The key of the map is the
	 *         parameter name, the values in the map are the parameter values.
	 */
	 ...
	 * outside that time, any information stored using this Map may no longer be
	 * kept by the container. Web socket applications running on distributed
	 * implementations of the web container should make any application specific
	 * objects stored here java.io.Serializable, or the object may not be recreated
	 * after a failover.
	 *
	...
	/**
	 * Return the authenticated user for this Session or {@code null} if no user is
	 ...
	 * connections to the same endpoint to which this session represents a
	 * connection. The Set includes the session this method is called on. These
	 * sessions may not still be open at any point after the return of this method.
	 * For example, iterating over the set at a later time may yield one or more
	 * closed sessions. Developers should use session.isOpen() to check.
	 *
