...
public class TyrusClientEngine implements ClientEngine {
	...
	/**
	 * Create {@link org.glassfish.tyrus.spi.WebSocketEngine} instance based on
	 * passed {@link WebSocketContainer} and with configured maximal incoming buffer
	 * size.
	 *
	 * @param endpointWrapper         wrapped client endpoint.
	 * @param listener                used for reporting back the outcome of
	 *                                handshake.
	 *                                {@link ClientHandshakeListener#onSessionCreated(javax.websocket.Session)}
	 *                                is invoked if handshake is completed and
	 *                                provided {@link Session} is open and ready to
	 *                                be returned from
	 *                                {@link WebSocketContainer#connectToServer(Class, javax.websocket.ClientEndpointConfig, java.net.URI)}
	 *                                (and alternatives) call.
	 * @param properties              passed container properties, see
	 *                                {@link org.glassfish.tyrus.client *
	...
	/* package */ TyrusClientEngine(TyrusEndpointWrapper endpointWrapper, ClientHandshakeListener listener,
		...
		MaskingKeyGenerator maskingKeyGenerator = Utils.getProperty(properties, ClientProperties.MASKING_KEY_GENERATOR,
				MaskingKeyGenerator.class, null);
	...
	public UpgradeRequest createUpgradeRequest(TimeoutHandler timeoutHandler) {
		...
		case INIT: {
			...
			clientHandShake = Handshake.createClientHandshake(RequestContext.Builder.create()
					.requestURI(connectToServerUriParam).secure("wss".equals(connectToServerUriParam.getScheme())).build());
		...
		case REDIRECT_REQUIRED: {
			...
			RequestContext requestContext = RequestContext.Builder.create(clientHandShake.getRequest()).requestURI(requestUri)
		...
		case AUTH_REQUIRED: {
			...
			if (clientEngineState.getAuthenticator() != null) {
				...
				if (LOGGER.isLoggable(Level.CONFIG)) {
					debugContext.appendLogMessage(LOGGER, Level.CONFIG, DebugContext.Type.MESSAGE_OUT,
							"Using " + "authenticator: ", clientEngineState.getAuthenticator().getClass().getName());
				}

				String authorizationHeader;
				try {
					final Credentials credentials = (Credentials) properties.get(ClientProperties.CREDENTIALS);
					debugContext.appendLogMessage(LOGGER, Level.CONFIG, DebugContext.Type.MESSAGE_OUT, "Using " + "credentials: ",
							credentials);
					authorizationHeader = clientEngineState.getAuthenticator().generateAuthorizationHeader(
							upgradeRequest.getRequestURI(), clientEngineState.getWwwAuthenticateHeader(), credentials);
	...
	@Override
	public ClientUpgradeInfo processResponse(final UpgradeResponse upgradeResponse, final Writer writer,
		...
		if (LOGGER.isLoggable(Level.FINE)) {
			...
			if (logUpgradeMessages) {
				debugContext.appendStandardOutputMessage(DebugContext.Type.MESSAGE_IN,
						"Received handshake response: " + "\n" + Utils.stringifyUpgradeResponse(upgradeResponse));
			}
		}
	...
	private ClientUpgradeInfo handleAuth(UpgradeResponse upgradeResponse) {
		...
		debugContext.appendLogMessage(LOGGER, Level.FINE, DebugContext.Type.MESSAGE_OUT,
				"Using " + "authentication config: ", authConfig);
		if (authConfig == null) {
			clientEngineState = TyrusClientEngineState.FAILED;
			listener.onError(new AuthenticationException(LocalizationMessages.AUTHENTICATION_FAILED()));
			return UPGRADE_INFO_FAILED;
		...
		debugContext.appendLogMessage(LOGGER, Level.FINE, DebugContext.Type.MESSAGE_OUT,
				"Using " + "authentication scheme: ", scheme);
	...
	private ClientUpgradeInfo handleRedirect(UpgradeResponse upgradeResponse) {
		if (!redirectEnabled) {
			clientEngineState = TyrusClientEngineState.FAILED;
			listener.onError(new RedirectException(upgradeResponse.getStatus(),
					LocalizationMessages.HANDSHAKE_HTTP_REDIRECTION_NOT_ENABLED(upgradeResponse.getStatus())));
			return UPGRADE_INFO_FAILED;
		}
		...
		if (locationString == null || locationString.equals("")) {
			listener.onError(new RedirectException(upgradeResponse.getStatus(),
					LocalizationMessages.HANDSHAKE_HTTP_REDIRECTION_NEW_LOCATION_MISSING()));
			clientEngineState = TyrusClientEngineState.FAILED;
			return UPGRADE_INFO_FAILED;
		}
		...
		try {
			...
			location = new URI(scheme, location.getUserInfo(), location.getHost(), port, location.getPath(),
			...
			if (!location.isAbsolute()) {
				...
				if (LOGGER.isLoggable(Level.FINEST)) {
					...
					LOGGER
							.finest("HTTP Redirect - Normalized and resolved Location URI header " + "against base URI: " + location);
				}
			}
	...
	private void logUpgradeRequest(UpgradeRequest upgradeRequest) {
		if (LOGGER.isLoggable(Level.FINE)) {
			...
			if (logUpgradeMessages) {
				debugContext.appendStandardOutputMessage(DebugContext.Type.MESSAGE_OUT,
						"Sending handshake " + "request:\n" + Utils.stringifyUpgradeRequest(upgradeRequest));
			}
		}
	}
	...
	/**
	 * Process upgrade response. This method should be called only when the response
	 * HTTP status code is {@code 101}.
	 *
	 * @param upgradeResponse upgrade response received from client container.
	 * @param writer          writer instance to be used for sending websocket
	 *                        frames.
	 ...
	 * @throws HandshakeException when there is a problem with passed
	 *                            {@link UpgradeResponse}.
	 */
	private ClientUpgradeInfo processUpgradeResponse(UpgradeResponse upgradeResponse, final Writer writer,
		...
		final List<Extension> handshakeResponseExtensions = TyrusExtension
				.fromHeaders(upgradeResponse.getHeaders().get(HandshakeRequest.SEC_WEBSOCKET_EXTENSIONS));
				...
				if (responseExtensionName != null && responseExtensionName.equals(installedExtension.getName())) {
					...
					if (!alreadyAdded) {
						if (installedExtension instanceof ExtendedExtension) {
							((ExtendedExtension) installedExtension).onHandshakeResponse(extensionContext,
									responseExtension.getParameters());
						}

						extensions.add(installedExtension);
						debugContext.appendLogMessage(LOGGER, Level.FINE, DebugContext.Type.OTHER, "Installed " + "extension: ",
		...
		final Session sessionForRemoteEndpoint = endpointWrapper.createSessionForRemoteEndpoint(socket,
				upgradeResponse.getFirstHeaderValue(HandshakeRequest.SEC_WEBSOCKET_PROTOCOL), extensions, debugContext);
		...
		// subprotocol and extensions are already set -- TODO: introduce new method
		// (onClientConnect)?
		...
		return new ClientUpgradeInfo() {
			...
			public Connection createConnection() {
				return new Connection() {
					...
					private final ReadHandler readHandler = new TyrusReadHandler(protocolHandler, socket, incomingBufferSize,
	...
	/**
	 * Get {@link TimeoutHandler} associated with current {@link ClientEngine}
	 * instance.
	 *
	...
	public static interface ClientHandshakeListener {
		...
		/**
		 * Invoked when handshake is completed and provided {@link Session} is open and
		 * ready to be returned from
		 * {@link WebSocketContainer#connectToServer(Class, javax.websocket.ClientEndpointConfig, java.net.URI)}
		 * (and alternatives) call.
		 *
	...
	/**
	 * State controls flow in
	 * {@link #processResponse(UpgradeResponse, Writer, Connection.CloseListener)}
	...
	private static enum TyrusClientEngineState {
		 ...
		 * Set in
		 * {@link #processResponse(UpgradeResponse, Writer, Connection.CloseListener)}
		 * when 3xx HTTP status code is received.
		 */
		REDIRECT_REQUIRED,
		 ...
		 * Set in
		 * {@link #processResponse(UpgradeResponse, Writer, Connection.CloseListener)}
		 * when 401 HTTP status code is received and the last upgrade request does not
		 * contain {@value UpgradeRequest#AUTHORIZATION} header (the last state was not
		 * {@link #AUTH_UPGRADE_REQUEST_CREATED}).
		 */
		AUTH_REQUIRED,

		/**
		 * Upgrade request with {@value UpgradeRequest#AUTHORIZATION} header has been
		 * created.
		 * <p/>
