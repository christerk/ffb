...
/**
 * Client thread pool configuration, which might be used to customize client
 * thread pool.
 * <p>
 * One can get a default <tt>ThreadPoolConfig</tt> using
 * {@link ThreadPoolConfig#defaultConfig()} and customize it according to the
 * application specific requirements.
 * <p>
 * A <tt>ThreadPoolConfig</tt> object might be customized in a "Builder"-like
 * fashion:
 * 
 * <pre>
...
public final class ThreadPoolConfig {
	...
	private static final ThreadPoolConfig DEFAULT = new ThreadPoolConfig("Tyrus-client", DEFAULT_CORE_POOL_SIZE,
			DEFAULT_MAX_POOL_SIZE, null, DEFAULT_MAX_QUEUE_SIZE, DEFAULT_IDLE_THREAD_KEEP_ALIVE_TIMEOUT, TimeUnit.SECONDS,
			null, Thread.NORM_PRIORITY, true, null);

	/**
	 * Create new client thread pool configuration instance. The returned
	 * <tt>ThreadPoolConfig</tt> instance will be pre-configured with a default
	 * values.
	 *
	...
	private ThreadPoolConfig(String poolName, int corePoolSize, int maxPoolSize, Queue<Runnable> queue, int queueLimit,
			long keepAliveTime, TimeUnit timeUnit, ThreadFactory threadFactory, int priority, boolean isDaemon,
	...
	/**
	 * Return a queue that will be used to temporarily store tasks when all threads
	 * in the thread pool are busy.
	 *
	 * @return queue that will be used to temporarily store tasks when all threads
	 *         in the thread pool are busy.
	 */
	...
	/**
	 * Set a queue implementation that will be used to temporarily store tasks when
	 * all threads in the thread pool are busy.
	 *
	 * @param queue queue implementation that will be used to temporarily store
	 *              tasks when all threads in the thread pool are busy.
	 * @return the {@link ThreadPoolConfig} with the new {@link java.util.Queue}
	 *         implementation.
	 */
	...
	/**
	 * Return {@link java.util.concurrent.ThreadFactory} that will be used to create
	 * thread pool threads.
	 * <p>
	 * If {@link java.util.concurrent.ThreadFactory} is set, then {@link #priority},
	 * {@link #isDaemon}, {@link #poolName} settings will not be considered when
	 * creating new threads.
	 *
	 * @return {@link java.util.concurrent.ThreadFactory} that will be used to
	...
	/**
	 * Set {@link java.util.concurrent.ThreadFactory} that will be used to create
	 * thread pool threads.
	 *
	 * @param threadFactory custom {@link java.util.concurrent.ThreadFactory} If
	 *                      {@link java.util.concurrent.ThreadFactory} is set, then
	 *                      {@link #priority}, {@link #isDaemon}, {@link #poolName}
	 *                      settings will not be considered when creating new
	 *                      threads.
	 * @return the {@link ThreadPoolConfig} with the new
	...
	/**
	 * Get priority of the threads in thread pool. The default is
	...
	/**
	 * Set priority of the threads in thread pool. The default is
	 * {@link Thread#NORM_PRIORITY}.
	 *
	...
	/**
	 * Return {@code true} if thread pool threads are daemons. The default is
	 * {@code true}.
	 *
	...
	/**
	 * Set {@code true} if thread pool threads are daemons. The default is
	 * {@code true}.
	 *
	...
	/**
	 * Get max thread pool size. The default is
	 * {@code Math.max(Runtime.getRuntime().availableProcessors(), 20)}
	 *
	...
	/**
	 * Get the core thread pool size - the size of the thread pool will never bee
	 * smaller than this.
	 * <p>
	 * The default is 1.
	 *
	 * @return the core thread pool size - the size of the thread pool will never
	 *         bee smaller than this.
	 */
	...
	/**
	 * Set the core thread pool size - the size of the thread pool will never bee
	 * smaller than this.
	 * <p>
	 * The default is 1.
	 *
	 * @param corePoolSize the core thread pool size - the size of the thread pool
	 *                     will never bee smaller than this.
	...
	/**
	 * Get the limit of the queue, where tasks are temporarily stored when all
	 * threads are busy.
	 * <p>
	...
	/**
	 * Set the limit of the queue, where tasks are temporarily stored when all
	 * threads are busy.
	 * <p>
	 * Value less than 0 means unlimited queue. The default is -1.
	 *
	 * @param queueLimit the thread pool queue limit. The <tt>queueLimit</tt> value
	 *                   less than 0 means unlimited queue.
	 ...
	 * <p>
	 * If the timeout expires and the thread is not a core one (see
	 * {@link #setCorePoolSize(int)}, {@link #setMaxPoolSize(int)}) - then the
	 * thread will be terminated and removed from the thread pool.
	 * <p>
	 * The default is 10s.
	 *
	...
	/**
	 * Get the class loader (if any) to be initially exposed by threads from this
	 * pool.
	 * <p>
	 * If not specified, the class loader of the parent thread that initialized the
	 * pool will be used.Authen
	 *
	 * @return the class loader (if any) to be initially exposed by threads from
	 *         this pool.
	 */
	 ...
	 * pool will be used.
	 *
	 * @param initialClassLoader the class loader to be exposed by threads of this
	 *                           pool.
	...
	public String toString() {
		return ThreadPoolConfig.class.getSimpleName() + " :\r\n" + "  poolName: " + poolName + "\r\n" + "  corePoolSize: "
				...
				+ (queue != null ? queue.getClass() : "undefined") + "\r\n" + "  queueLimit: " + queueLimit + "\r\n"
