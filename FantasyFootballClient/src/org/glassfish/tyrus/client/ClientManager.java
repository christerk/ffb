...
public class ClientManager extends BaseContainer implements WebSocketContainer {
	 ...
	 * <p>
	 * Value must be {@code int} and represents handshake timeout in milliseconds.
	 * Default value is 30000 (30 seconds).
	 *
	 * @deprecated please use
	 *             {@link org.glassfish.tyrus.client.ClientProperties#HANDSHAKE_TIMEOUT}.
	 */
	 ...
	 * <p>
	 * Value must be
	 * {@link org.glassfish.tyrus.client.ClientManager.ReconnectHandler} instance.
	 *
	 * @deprecated please use
	 *             {@link org.glassfish.tyrus.client.ClientProperties#RECONNECT_HANDLER}.
	 */
	 ...
	 * <p>
	 * Value is expected to be {@link String} and represent proxy URI. Protocol part
	 * is currently ignored but must be present ({@link URI#URI(String)} is used for
	 * parsing).
	 * 
	 * <pre>
	 ...
	 * @deprecated please use
	 *             {@link org.glassfish.tyrus.client.ClientProperties#PROXY_URI}.
	 */
	 ...
	 * <p>
	 * Value is expected to be {@link Map}&lt;{@link String}, {@link String}&gt; and
	 * represent raw http headers to be added to initial request which is sent to
	 * proxy. Key corresponds to header name, value is header value.
	 * <p>
	 * Sample below demonstrates use of this feature to set preemptive basic proxy
	 * authentication:
	 * 
	 * <pre>
	 ...
	 * </pre>
	 * 
	 * Please note that these headers will be used only when establishing proxy
	 * connection, for modifying WebSocket handshake headers, see
	 * {@link javax.websocket.ClientEndpointConfig.Configurator#beforeRequest(java.util.Map)}.
	 *
	 * @see javax.websocket.ClientEndpointConfig#getUserProperties()
	 * @deprecated please use
	 *             {@link org.glassfish.tyrus.client.ClientProperties#PROXY_HEADERS}.
	 */
	 ...
	 * {@code org.glassfish.grizzly.ssl.SSLEngineConfigurator} when configuring
	 * Grizzly client or {@link org.glassfish.tyrus.client.SslEngineConfigurator}
	 ...
	 *
	 * @deprecated please use
	 *             {@link org.glassfish.tyrus.client.ClientProperties#SSL_ENGINE_CONFIGURATOR}.
	 */
	 ...
	 * <p>
	 * Uses {@link ClientManager#CONTAINER_PROVIDER_CLASSNAME} as container
	 * implementation, thus relevant module needs to be on classpath. Setting
	 * different container is possible via
	 * {@link ClientManager#createClient(String)}.
	 *
	 ...
	 *
	 * @param containerProviderClassName classname of container provider. It will be
	 *                                   loaded using context class loader.
	...
	private ClientManager(String containerProviderClassName, WebSocketContainer webSocketContainer) {
		...
		// TODO this could be replaced by a "standalone" counter and one method. Keep or
		// get rid of?
				...
				if (activeClientCounter.decrementAndGet() == 0) {
					ClientManager.this.shutdown(new ShutDownCondition() {
						...
						public boolean evaluate() {
							/*
							 * the condition is evaluated in synchronized block -> check that nothing
							 * changed while the thread was waiting for the lock
							 */
	...
	public Session connectToServer(Class annotatedEndpointClass, URI path) throws DeploymentException, IOException {
		if (annotatedEndpointClass.getAnnotation(ClientEndpoint.class) == null) {
			throw new DeploymentException(
					String.format("Class argument in connectToServer(Class, URI) is to be annotated endpoint class. Class "
	 ...
	 * <p>
	 * Only simple checks are performed in the main thread; client container is
	 * created in different thread, same applies to connecting etc.
	 *
	 ...
	 * @throws DeploymentException if the class is not a valid annotated endpoint
	 *                             class.
	 */
	...
	/**
	 * Non-blocking version of
	 * {@link WebSocketContainer#connectToServer(Class, javax.websocket.ClientEndpointConfig, java.net.URI)}.
	 * <p>
	 * Only simple checks are performed in the main thread; client container is
	 * created in different thread, same applies to connecting etc.
	 *
	 ...
	 * created in different thread, same applies to connecting etc.
	 *
	 * @param endpointInstance the programmatic client endpoint instance
	 *                         {@link Endpoint}.
	 * @param path             the complete path to the server endpoint.
	 * @param cec              the configuration used to configure the programmatic
	 *                         endpoint.
	 * @return the Session created if the connection is successful.
	 * @throws DeploymentException if the configuration is not valid
	 * @see WebSocketContainer#connectToServer(javax.websocket.Endpoint,
	 *      javax.websocket.ClientEndpointConfig, java.net.URI)
	 */
	...
	/**
	 * Non-blocking version of
	 * {@link WebSocketContainer#connectToServer(Object, java.net.URI)}.
	 * <p>
	 * Only simple checks are performed in the main thread; client container is
	 * created in different thread, same applies to connecting etc.
	 *
	 ...
	 * @param synchronous   if {@code true} connect will be executed synchronously,
	 *                      if {@code false} connect will be executed
	 *                      asynchronously.
	...
	Future<Session> connectToServer(final Object o, final ClientEndpointConfig configuration, final String url,
		...
		/*
		 * Client activity listener must be called before an executor service is
		 * obtained from the container, otherwise the counter of active client won't be
		 * increased and the executor service might be shut down
		 */
		...
		final TyrusFuture<Session> future = new TyrusFuture<Session>() {
			...
			public void setFailure(Throwable throwable) {
				...
				// make sure that the number of active clients decreases each time an attempt to
				// connect fails
		...
		executorService.submit(new Runnable() {
			...
			public void run() {
				...
				Integer tyrusIncomingBufferSize = Utils.getProperty(copiedProperties, ClientProperties.INCOMING_BUFFER_SIZE,
						Integer.class);
					...
					if (o instanceof Endpoint) {
						...
						if (Endpoint.class.isAssignableFrom((Class<?>) o)) {
							...
							collector.addException(new DeploymentException(String.format(
									"Class %s in not Endpoint descendant and does not have @ClientEndpoint", ((Class<?>) o).getName())));
							endpoint = null;
							config = null;
						}
					} else {
				...
				final boolean retryAfterEnabled = Utils.getProperty(copiedProperties,
						ClientProperties.RETRY_AFTER_SERVICE_UNAVAILABLE, Boolean.class, false);
				final ReconnectHandler userReconnectHandler = Utils.getProperty(copiedProperties,
						ClientProperties.RECONNECT_HANDLER, ReconnectHandler.class);

				final Runnable connector = new Runnable() {

					private final ReconnectHandler reconnectHandler = retryAfterEnabled
	 ...
	 * <p>
	 * Used to set container specific configuration as SSL truststore and keystore,
	 * HTTP Proxy configuration and maximum incoming buffer size. These properties
	 * cannot be shared among various containers due to constraints in WebSocket
	 * API, so if you need to have multiple configurations, you will need to create
	 * multiple ClientManager instances or synchronize connectToServer method
	 * invocations.
	 *
	...
	/**
	 * Executor service which just executes provided {@link Runnable} in the very
	 * same thread.
	 */
	 ...
	 * Reconnect handler.
	 * <p>
	 * When implementing, be sure that you do have enough logic behind cancelling
	 * reconnect feature - even {@link javax.websocket.Session#close()} call will be
	 * treated just like any other disconnect resulting in reconnect.
	 */
	public static class ReconnectHandler {
		...
		/**
		 * Called after {@link javax.websocket.OnClose} annotated method (or
		 ...
		 * <p>
		 * Type of the failure is indicated by {@link Exception} parameter. Be cautious
		 * when implementing this method, you might easily cause DDoS like behaviour.
		 *
		 ...
		 * Get reconnect delay.
		 * <p>
		 * Called after {@link #onDisconnect(CloseReason)} or
		 * {@link #onConnectFailure(Exception)} when {@code true} is returned. When
		 * positive value is returned, next connection attempt will be made after that
		 * number of seconds.
		 * <p>
	...
	/**
	 * A listener listening for events that indicate when a client has started to be
	 * active and when it stopped to be active. It is used to determine the number
	 * of active clients.
	 */
	private static interface ClientActivityListener {
		 ...
		 * <p>
		 * This should be called either when the session is closed or an attempt to
