...
/**
 * WebSocket engine is used for upgrading HTTP requests into websocket
 * connections. A transport gets hold of the engine from the
...
public interface WebSocketEngine {
	...
	/**
	 * Upgrade info that includes status for HTTP request upgrading and connection
	 * creation details.
	 */
	interface UpgradeInfo {
		...
		/**
		 * Creates a connection if the upgrade is successful. Tyrus would call onConnect
		 * lifecycle method on the endpoint during the invocation of this method.
		 *
	...
	enum UpgradeStatus {
		/**
		 * Not a WebSocketRequest or no mapping in the application. This may mean that
		 * HTTP request processing should continue (in servlet container, the next
		 * filter may be called).
		 */
		NOT_APPLICABLE,

		/**
		 * Upgrade failed due to version, extensions, origin check etc. Tyrus would set
		 * an appropriate HTTP error status code in {@link UpgradeResponse}.
		 */
		HANDSHAKE_FAILED,
