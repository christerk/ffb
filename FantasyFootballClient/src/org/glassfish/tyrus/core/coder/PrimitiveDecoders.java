...
public abstract class PrimitiveDecoders<T> extends CoderAdapter implements Decoder.Text<T> {
	...
	static {
		ALL = Collections.unmodifiableList(
				Arrays.<Class<? extends Decoder>>asList(BooleanDecoder.class, ByteDecoder.class, CharacterDecoder.class,
						DoubleDecoder.class, FloatDecoder.class, IntegerDecoder.class, LongDecoder.class, ShortDecoder.class));
	...
	private static Map<Class<?>, Decoder.Text<?>> getAllInstances() {
		...
		for (Class<? extends Decoder> dec : ALL) {
			...
			try {
				...
				Logger.getLogger(PrimitiveDecoders.class.getName()).log(Level.WARNING,
						String.format("Decoder %s could not have been instantiated.", dec));
			}
		}
