 ...
 * <p>
 * There is one {@link TyrusEndpointWrapper} for each application class, which
...
public class TyrusEndpointWrapper {
	...
	/**
	 * Used as threshold for parallel broadcast. When the sessions are divided
	 * between threads, number of sessions per thread should not be lower than this
	 * constant.
	 */
	 ...
	 *
	 * @param endpointClass            endpoint class for which the wrapper is
	 *                                 created.
	 ...
	 * @param clusterContext           cluster context instance. {@code null}
	 *                                 indicates standalone mode.
	 * @param endpointEventListener    endpoint event listener.
	 * @param parallelBroadcastEnabled {@code true} if parallel broadcast should be
	 *                                 enabled, {@code true} is default.
	...
	public TyrusEndpointWrapper(Class<? extends Endpoint> endpointClass, EndpointConfig configuration,
			ComponentProviderService componentProvider, WebSocketContainer container, String contextPath,
			ServerEndpointConfig.Configurator configurator, SessionListener sessionListener, ClusterContext clusterContext,
			EndpointEventListener endpointEventListener, Boolean parallelBroadcastEnabled) throws DeploymentException {
		this(null, endpointClass, configuration, componentProvider, container, contextPath, configurator, sessionListener,
				clusterContext, endpointEventListener, parallelBroadcastEnabled);
	}

	/**
	 * Create {@link TyrusEndpointWrapper} for {@link Endpoint} instance or
	 * {@link AnnotatedEndpoint} instance.
	 *
	 * @param endpoint                 endpoint instance for which the wrapper is
	 *                                 created.
	 * @param configuration            endpoint configuration.
	 * @param componentProvider        component provider.
	 * @param container                container where the wrapper is running.
	...
	private TyrusEndpointWrapper(Endpoint endpoint, Class<? extends Endpoint> endpointClass, EndpointConfig configuration,
			ComponentProviderService componentProvider, WebSocketContainer container, String contextPath,
			final ServerEndpointConfig.Configurator configurator, SessionListener sessionListener,
			final ClusterContext clusterContext, EndpointEventListener endpointEventListener,
			Boolean parallelBroadcastEnabled) throws DeploymentException {
		...
		this.componentProvider = configurator == null ? componentProvider
		...
		if (clusterContext != null) {
			clusterContext.registerSessionListener(getEndpointPath(), new org.glassfish.tyrus.core.cluster.SessionListener() {
				...
				public void onSessionOpened(String sessionId) {
					final Map<RemoteSession.DistributedMapKey, Object> distributedSessionProperties = clusterContext
							.getDistributedSessionProperties(sessionId);
					clusteredSessions.put(sessionId, new RemoteSession(sessionId, clusterContext, distributedSessionProperties,
							TyrusEndpointWrapper.this, dummySession));
				}
			...
			for (String sessionId : clusterContext.getRemoteSessionIds(getEndpointPath())) {
				...
				clusteredSessions.put(sessionId,
						new RemoteSession(sessionId, clusterContext, distributedSessionProperties, this, dummySession));
			}
		}
	}
	...
	private <T> Object getCoderInstance(Session session, CoderWrapper<T> wrapper) {
		...
		if (coder == null) {
			...
			final Object coderInstance = this.componentProvider.getCoderInstance(wrapper.getCoderClass(), session,
					getEndpointConfig(), collector);
	...
	Object decodeCompleteMessage(TyrusSession session, Object message, Class<?> type,
		...
		if (Decoder.Text.class.isAssignableFrom(decoderClass)) {
			if (type != null && type.isAssignableFrom(selectedDecoder.getType())) {
				...
				session.getDebugContext().appendLogMessage(LOGGER, Level.FINEST, DebugContext.Type.MESSAGE_IN, "Decoding with ",
						selectedDecoder);
	...
	private ArrayList<CoderWrapper<Decoder>> findApplicableDecoders(TyrusSession session, Object message,
		...
		session.getDebugContext().appendLogMessage(LOGGER, Level.FINEST, DebugContext.Type.MESSAGE_IN,
				"Applicable decoders: ", result);
	...
	private void logUsedEncoder(CoderWrapper<Encoder> encoder, Session session) {
		if (LOGGER.isLoggable(Level.FINEST)) {
			if (session instanceof TyrusSession) {
				((TyrusSession) session).getDebugContext().appendLogMessage(LOGGER, Level.FINEST, DebugContext.Type.MESSAGE_OUT,
	 ...
	 * <p>
	 * In this context, server endpoint path is exactly what is present in
	 * {@link javax.websocket.server.ServerEndpoint} annotation or returned from
	 * {@link javax.websocket.server.ServerEndpointConfig#getPath()} method call.
	 * Context path is not included.
	 *
	...
	/**
	 * Server-side; Get the negotiated extensions' names based on the extensions
	 * supported by client.
	 *
	 * @param clientExtensions names of the extensions' supported by client.
	 * @return names of extensions supported by both client and class that
	 *         implements this one.
	 */
	List<Extension> getNegotiatedExtensions(List<Extension> clientExtensions) {
		if (configuration instanceof ServerEndpointConfig) {
			return configurator.getNegotiatedExtensions(((ServerEndpointConfig) configuration).getExtensions(),
					clientExtensions);
		} else {
			return Collections.emptyList();
		}
	...
	/**
	 * Creates a Session based on the {@link TyrusWebSocket}, subprotocols and
	 * extensions.
	 *
	...
	public Session createSessionForRemoteEndpoint(TyrusWebSocket socket, String subprotocol, List<Extension> extensions,
		...
		final TyrusSession session = new TyrusSession(container, socket, this, subprotocol, extensions, false,
				getURI(contextPath, null), null, Collections.<String, String>emptyMap(), null,
				Collections.<String, List<String>>emptyMap(), null, null, null, debugContext);
	 ...
	 *
	 * @param socket         {@link TyrusWebSocket} who has just connected to this
	 *                       web socket endpoint.
	 * @param upgradeRequest request associated with accepted connection.
	 * @param debugContext   debug context.
	 * @return Created {@link Session} instance or {@code null} when session was not
	 *         created properly (max sessions limit on endpoint or application or
	...
	Session onConnect(TyrusWebSocket socket, UpgradeRequest upgradeRequest, String subProtocol,
		...
		if (session == null) {
			...
			if (maxSessionPerEndpointExceeded || !onOpenResult.equals(SessionListener.OnOpenResult.SESSION_ALLOWED)) {
				try {
					...
					debugContext.appendLogMessage(LOGGER, Level.FINE, DebugContext.Type.MESSAGE_IN, "Session opening refused: ",
							refuseDetail);
		...
		// TYRUS-325: Server do not close session properly if non-instantiable endpoint
		// class is provided
	...
	/**
	 * Called by the provider when the web socket connection has an incoming text
	 * message from the given remote endpoint.
	 *
	...
	/**
	 * Called by the provider when the web socket connection has an incoming partial
	 * text message from the given remote endpoint. Partial text messages are passed
	 * in sequential order, one piece at a time. If an implementation does not
	 * support streaming, it will need to reconstruct the message here and pass the
	 * whole thing along.
	 *
	 * @param socket        {@link TyrusWebSocket} who sent the message.
	 * @param partialString the String message.
	 * @param last          to indicate if this is the last partial string in the
	 *                      sequence
	 */
	...
	/**
	 * Called by the provider when the web socket connection has an incoming partial
	 * binary message from the given remote endpoint. Partial binary messages are
	 * passed in sequential order, one piece at a time. If an implementation does
	 * not support streaming, it will need to reconstruct the message here and pass
	 * the whole thing along.
	 *
	 * @param socket       {@link TyrusWebSocket} who sent the message.
	 * @param partialBytes the piece of the binary message.
	 * @param last         to indicate if this is the last partial byte buffer in
	 *                     the sequence
	 */
	void onPartialMessage(TyrusWebSocket socket, ByteBuffer partialBytes, boolean last) {
		...
		session.getDebugContext().appendLogMessage(LOGGER, Level.FINEST, DebugContext.Type.MESSAGE_IN,
				"Received partial binary message");
	...
	/**
	 * Check {@link Throwable} produced during {@link javax.websocket.OnMessage}
	 * annotated method call.
	 *
	 * @param throwable thrown {@link Throwable}.
	 * @param session   {@link Session} related to {@link Throwable}.
	 * @return {@code true} when exception is handled within this method (framework
	 *         produced it), {@code false} otherwise.
	 */
	private boolean processThrowable(Throwable throwable, Session session) {
		if (LOGGER.isLoggable(Level.FINE)) {
			LOGGER.log(Level.FINE, String.format("Exception thrown while processing message. Session: '%session'.", session),
					throwable);
		}
	...
	/**
	 * Called by the provider when the web socket connection has an incoming pong
	 * message from the given remote endpoint.
	 *
	 * @param socket {@link TyrusWebSocket} who sent the message.
	...
	void onPong(TyrusWebSocket socket, final ByteBuffer bytes) {
		...
		if (session.isPongHandlerPresent()) {
			...
			session.getDebugContext().appendLogMessage(LOGGER, Level.FINEST, DebugContext.Type.MESSAGE_IN,
					"Unhandled pong message");
		}
	}

	/**
	 * Called by the provider when the web socket connection has an incoming ping
	 * message from the given remote endpoint.
	 * <p>
	 * The endpoint needs to respond as soon as possible (see the websocket RFC). No
	 * involvement from application layer, there is no ping listener.
	 *
	 * @param socket {@link TyrusWebSocket} who sent the message.
	 * @param bytes  the message.
	 */
	void onPing(TyrusWebSocket socket, ByteBuffer bytes) {
		...
		try {
			...
			// do nothing.
			// we might consider calling onError, but there should be better defined
			// exception.
		}
	}

	/**
	 * Called by the provider when the web socket connection to the given remote
	 * endpoint has just closed.
	 *
	 ...
	 * @return map of sessions and futures for user to get the information about
	 *         status of the message. Messages send from other cluster nodes are not
	 *         included.
	 */
	 ...
	 * Perform broadcast.
	 *
	 * @param broadcastCallable callable which serializes a frame to byte[] and
	 *                          passes it to
	 *                          {@link org.glassfish.tyrus.core.TyrusWebSocket}. The
	 *                          {@link org.glassfish.tyrus.core.TyrusEndpointWrapper.SessionCallable}
	 *                          also handles extensions, if any.
	...
	/**
	 * Serialize dataframe and send it to
	 * {@link org.glassfish.tyrus.core.TyrusWebSocket}.
	 *
	 * @param webSocket {@link org.glassfish.tyrus.core.TyrusWebSocket} instance
	...
	/**
	 * Divides open sessions into subsets on which an operation passed as
	 * {@code broadcastCallable} will be executed in parallel.
	 *
	...
	private void handleSubmitException(Map<Session, Future<?>> futures,
			List<Map.Entry<TyrusWebSocket, TyrusSession>> sessions, int[] bounds, Exception e) {
	...
	/**
	 * Server side check for protocol specific information to determine whether the
	 * request can be upgraded.
	 * <p>
	 * The default implementation will check for the presence of the {@code Upgrade}
	 * header with a value of {@code WebSocket}.
	 *
	 * @param request received {@link UpgradeRequest}.
	 * @return {@code true} if the request should be upgraded to a WebSocket
	 *         connection.
	 * @throws HandshakeException when origin verification check returns
	 *                            {@code false}.
	 */
	 ...
	 * wish to override this to return customized {@link TyrusWebSocket}
	 * implementations.
	 *
	...
	/**
	 * This method will be invoked if an unexpected exception is caught by the
	 * WebSocket runtime.
	 *
	 * @param socket the websocket being processed at the time the exception
	 *               occurred.
	...
	boolean onError(TyrusWebSocket socket, Throwable t) {
		Logger.getLogger(TyrusEndpointWrapper.class.getName()).log(Level.WARNING,
				LocalizationMessages.UNEXPECTED_ERROR_CONNECTION_CLOSE(), t);
		return true;
	}
	 ...
	 * Session listener.
	 * <p>
	 * TODO: rename/consolidate with
	 * {@link org.glassfish.tyrus.core.monitoring.EndpointEventListener}?
	 */
	public abstract static class SessionListener {

		/**
		 * Result of
		 * {@link org.glassfish.tyrus.core.TyrusEndpointWrapper.SessionListener#onOpen(TyrusSession)}.
		 */
		public enum OnOpenResult {
			...
			/**
			 * Session cannot be opened - the maximal number of open session per application
			 * exceeded.
			 */
			MAX_SESSIONS_PER_APP_EXCEEDED,

			/**
			 * Session cannot be opened - the maximal number of open session per remote
			 * address exceeded.
			 */
		...
		/**
		 * Invoked before {@link javax.websocket.OnOpen} annotated method or
		 * {@link Endpoint#onOpen(javax.websocket.Session, javax.websocket.EndpointConfig)}
		 * is invoked.
		 * <p>
		 * Default implementation always returns
		 * {@link org.glassfish.tyrus.core.TyrusEndpointWrapper.SessionListener.OnOpenResult#SESSION_ALLOWED}.
		 *
		...
		/**
		 * Invoked after {@link javax.websocket.OnClose} annotated method or
	...
	/**
	 * Used only as a placeholder to get encoder instances from
