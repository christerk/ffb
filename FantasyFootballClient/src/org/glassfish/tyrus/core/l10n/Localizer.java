...
/**
 * Localizes the {@link Localizable} into a message by using a configured
 * {@link Locale}.
 *
		...
		try {
			...
			if (bundle == null) {
				try {
					...
					// all files with an extension different from .class (hence all the .properties
					// files)
					// get copied to the top level directory instead of being in the package where
					// they
					// are defined
					// so, since we can't find the bundle under its proper name, we look for it
					// under
...
//                                }
			...
			try {
				...
				// notice that this may throw a MissingResourceException of its own (caught
				// below)
