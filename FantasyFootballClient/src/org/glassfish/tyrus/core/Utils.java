...
public class Utils {
	...
	/**
	 * Creates the array of bytes containing the bytes from the position to the
	 * limit of the {@link ByteBuffer}.
	 *
	 * @param buffer where the bytes are taken from.
	 * @return array of bytes containing the bytes from the position to the limit of
	 *         the {@link ByteBuffer}.
	 */
	...
	/**
	 * Creates single {@link String} value from provided List by calling
	 * {@link Object#toString()} on each item and separating existing ones with
	 * {@code ", "}.
	 *
	 ...
	 * @param stringifier strignifier used for conversion. When {@code null},
	...
	/**
	 * Check for null. Throws {@link IllegalArgumentException} if provided value is
	 * null.
	 *
	 * @param reference     object to check.
	 * @param parameterName name of parameter to be formatted into localized message
	 *                      of thrown {@link IllegalArgumentException}.
	 * @param <T>           object type.
	 */
	...
	/**
	 * Concatenates two buffers into one. If buffer given as first argument has
	 * enough space for putting the other one, it will be done and the original
	 * buffer will be returned. Otherwise new buffer will be created.
	 *
	 * @param buffer             first buffer.
	 * @param buffer1            second buffer.
	 * @param incomingBufferSize incoming buffer size. Concatenation length cannot
	 *                           be bigger than this value.
	 * @param BUFFER_STEP_SIZE   buffer step size.
	 * @return concatenation.
	 * @throws IllegalArgumentException when the concatenation length is bigger than
	 *                                  provided incoming buffer size.
	 */
	public static ByteBuffer appendBuffers(ByteBuffer buffer, ByteBuffer buffer1, int incomingBufferSize,
		...
		if (len < (capacity - limit)) {
			...
			if (newSize > incomingBufferSize) {
				...
				final int roundedSize = (newSize % BUFFER_STEP_SIZE) > 0 ? ((newSize / BUFFER_STEP_SIZE) + 1) * BUFFER_STEP_SIZE
						: newSize;
	 ...
	 * @param type         type of value to be retrieved.
	 * @param <T>          type of value to be retrieved.
	 * @param defaultValue value returned when record does not exist in supplied
	 *                     map.
	 * @return typed value or {@code null} if property is not set or value is not
	 *         assignable.
	 */
	 ...
	 * <p>
	 * Expected schemes are {@code "ws"} and {@code "wss"} and this method will
	 * return {@code 80} or {@code 443} when the port is not explicitly set in the
	 * provided {@link URI}.
	 *
	 ...
	 *
	 * @param uri    provided uri.
	 * @param scheme scheme to be used when checking for {@code "ws"} and
	 *               {@code "wss"}.
	 * @return port number which should be used for creating connections/etc.
	 */
	 ...
	 * <li>{@code Sunday, 06-Nov-94 08:49:37 GMT} (RFC 850, obsoleted by RFC
	 * 1036)</li>
	 ...
	 * @throws ParseException if the specified string cannot be parsed in neither of
