 ...
 * <p>
 * Responsible for framing and unframing raw websocket frames. Tyrus creates
 * exactly one instance per Session.
 */
public final class ProtocolHandler {
	 ...
	 * <p>
	 * The reason for this lock is that we need to have consistent value in
	 * {#sendingFragment} field to be able to determine the sending state of this
	 * particular instance/session.
	 */
	private final Lock lock = new ReentrantLock();

	/**
	 * If partial message is being send and we want to send partial message with
	 * different type or other whole message, we need to wait until "idleCondition"
	 * is signalled.
	 */
	 ...
	 * Constructor.
	 *
	 * @param client              {@code true} when this instance is on client side,
	 *                            {@code false} when on server side.
	 * @param maskingKeyGenerator random number generator that will be used for
	 *                            generating masking keys. Masking keys are required
	 *                            only on the client side and
	 *                            {@code maskingKeyGenerator} should be {@code
	 *                            null} on the server side. If {@code null} on the
	 *                            client side, {@link java.security.SecureRandom}
	 ...
	 *
	 * @return {@code true} if there is at least one negotiated extension associated
	 *         to this connection, {@code false} otherwise.
	 */
	 ...
	 *
	 * @param endpointWrapper  endpoint related to the handshake (path is already
	 *                         matched).
	 * @param request          handshake request.
	 * @param response         handshake response.
	 * @param extensionContext extension context.
	 * @return server handshake object.
	 * @throws HandshakeException when there is problem with received
	 *                            {@link UpgradeRequest}.
	 */
	public Handshake handshake(TyrusEndpointWrapper endpointWrapper, UpgradeRequest request, UpgradeResponse response,
	 ...
	 * <p>
	 * If yes, wait for {@value ProtocolHandler#SEND_TIMEOUT} and if the message
	 * still cannot be sent, throw {@link IllegalStateException}.
	 */
	private void checkSendingFragment() {
		final long timeout = System.currentTimeMillis() + SEND_TIMEOUT;

		// idleCondition can be signalled but other thread could be scheduled before
		// this one; of that thread starts
		// sending another partial message, we should wait again for the condition to be
		// signalled.
	 ...
	 * <li>up to 0xFFFF, 3 element array starting with 126 with the following 2
	 * bytes interpreted as a 16 bit unsigned integer showing the payload length.
	 * <li>else 9 element array starting with 127 with the following 8 bytes
	 * interpreted as a 64-bit unsigned integer (the high bit must be 0) showing the
	 * payload length.
	 * </ol>
	 *
	...
	/* package */ ByteBuffer frame(Frame frame) {
		...
		// TODO - in that case, we will need to NOT store dataframe inmemory - introduce
		// maskingByteStream or
	...
	public Frame unframe(ByteBuffer buffer) {
		...
		try {
			// this do { .. } while cycle was forced by findbugs check - complained about
			...
			do {
				...
				case 3:
					...
					final Frame frame = Frame.builder().fin(parsingState.finalFragment).rsv1(isBitSet(parsingState.opcode, 6))
							.rsv2(isBitSet(parsingState.opcode, 5)).rsv3(isBitSet(parsingState.opcode, 4))
							.opcode((byte) (parsingState.opcode & 0xf)).payloadLength(parsingState.length).payloadData(data).build();
	...
	public void process(Frame frame, TyrusWebSocket socket) {
		...
		// TODO - investigate whether it can be removed; (this effectively denies lazy
		// decoding)
		...
		// (SERVICE_RESTART and TRY_AGAIN_LATER does not make sense from the client
		// side.
		if (!client) {
