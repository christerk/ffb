...
/**
 * A {@link java.util.logging.Logger} wrapper that gives logging records a
 * common formatting and temporarily stores log records and postpones their
 * logging until they can be provided with a session ID. After a session ID has
 * been provided, messages are logged immediately.
 * <p>
 * Log records are provided with a session ID, so that log records from a single
 * upgrade request can be easily linked together in a log of a busy server or
 * client.
 *
...
public class DebugContext {
	...
	// is not thread safe - it is assumed that it will be used only in the
	// "handshake phase"
	...
	/**
	 * Constructor that configures tracing to be ON and accepts tracing threshold as
	 * a parameter.
	 *
	...
	/**
	 * Append a message to the log, the logging will be postponed until the message
	 * can be provided with a session ID. Randomly generated session ID is used if a
	 * session has not been created.
	 *
	 * @param logger       logger to be used to log the message.
	 * @param loggingLevel message level.
	 * @param type         type of the message.
	 * @param messageParts message parts that will be concatenated to create a log
	 *                     message.
	 */
	...
	/**
	 * Append a message to the log and to the list of trace messages that are sent
	 * in handshake response. The logging will be postponed until the message can be
	 * provided with a session ID. Randomly generated session ID is used if a
	 * session has not been created.
	 *
	 * @param logger       logger to be used to log the message.
	 * @param loggingLevel message level.
	 * @param type         type of the message.
	 * @param messageParts message parts that will be stringified and concatenated
	...
	/**
	 * Write a message to the standard output, the logging will be postponed until
	 * the message can be provided with a session ID. Randomly generated session ID
	...
	/**
	 * Set a session ID that will be used as a common identifier for logged messages
	 * related to the same upgrade request. Setting the session ID will cause the
	 * pending messages to be written into the log.
	 *
	...
	public void flush() {
		if (sessionId == null) {
			// something went wrong before the session could have been initialized, just
			// give all the messages some id.
			...
			if (logRecord.printToSout) {
				...
				if (logRecord.t != null) {
					logRecord.logger.log(logRecord.loggingLevel,
							formatLogMessage(logRecord.message, logRecord.type, logRecord.timestamp), logRecord.t);
				} else {
					logRecord.logger.log(logRecord.loggingLevel,
	...
	private static class LogRecord {
		...
		private Throwable t;
		/**
		 * {@code true} if the record should be printed into standard output - used if a
		 * message should be "logged" even though the logging has been turned off or the
		 * configured logging level is too high.
		 */
	...
	/**
	 * Type of the record - used to graphically distinguish these message types in
	 * the log.
	 */
	public enum Type {
		MESSAGE_IN, MESSAGE_OUT, OTHER
	}
	...
	public enum TracingType {
		...
		OFF,
		/**
		 * Tracing headers will be sent in handshake response only if
		 * X-Tyrus-Tracing-Accept header is present in handshake request.
		 */
		ON_DEMAND,
		/**
	...
	/**
	 * Tracing threshold - used for configuration granularity of information that
	 * will be sent in tracing headers.
	 */
	public enum TracingThreshold {
		/**
		 * A less verbose tracing, an equivalent to
		 * {@link java.util.logging.Level#FINER} logging level.
		 */
		SUMMARY,
		/**
		 * A more verbose tracing, an equivalent to {@link java.util.logging.Level#FINE}
		 * logging level.
		 * <p>
