...
public final class Handshake {
	 ...
	 *
	 * @param webSocketRequest request representation to be modified for use as
	...
	/**
	 * Client side only - compose the {@link UpgradeRequest} and store it for
	 * further use.
	 *
	 ...
	 * @throws HandshakeException when HTTP Status of received response is not 101 -
	 *                            Switching protocols.
	 */
	public void validateServerResponse(UpgradeResponse response) throws HandshakeException {
		if (RESPONSE_CODE_VALUE != response.getStatus()) {
			throw new HandshakeException(response.getStatus(),
...
//        }
	...
	/**
	 * Client side only - Generate host and origin header and put them to the
	...
	static Handshake createServerHandshake(UpgradeRequest request, ExtendedExtension.ExtensionContext extensionContext)
		...
		handshake.subProtocols = (protocolHeader == null ? Collections.<String>emptyList()
...
//        }
	...
	/* ,TyrusUpgradeResponse response */) {
		...
		final List<Extension> extensions = TyrusExtension
				.fromString(request.getHeaders().get(HandshakeRequest.SEC_WEBSOCKET_EXTENSIONS));
		...
		if (!negotiatedExtensions.isEmpty()) {
			response.getHeaders().put(HandshakeRequest.SEC_WEBSOCKET_EXTENSIONS,
