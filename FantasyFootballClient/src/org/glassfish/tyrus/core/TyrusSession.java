...
public class TyrusSession implements Session, DistributedSession {
	...
	TyrusSession(WebSocketContainer container, TyrusWebSocket socket, TyrusEndpointWrapper endpointWrapper,
			String subprotocol, List<Extension> extensions, boolean isSecure, URI requestURI, String queryString,
			Map<String, String> pathParameters, Principal principal, Map<String, List<String>> requestParameterMap,
			final ClusterContext clusterContext, String connectionId, final String remoteAddr, DebugContext debugContext) {
		...
		if (clusterContext != null) {
			...
			distributedPropertyMap.put(RemoteSession.DistributedMapKey.MAX_BINARY_MESSAGE_BUFFER_SIZE,
					maxBinaryMessageBufferSize);
			distributedPropertyMap.put(RemoteSession.DistributedMapKey.MAX_TEXT_MESSAGE_BUFFER_SIZE,
	 ...
	 * <p>
	 * Remote sessions are websocket sessions which are bound to another node in the
	 * cluster.
	 *
	 * @return set of remote sessions or empty set, when not running in cluster
	 *         environment.
	 */
	 ...
	 * <p>
	 * Set returned from this method contains all "local" and {@link RemoteSession
	 * remote} sessions (if any).
	 *
	 ...
	 * @return map of local sessions and futures for user to get the information
	 *         about status of the message.
	 */
	...
	/**
	 * Broadcasts binary message to all connected clients, including remote sessions
	 * (if any).
	 * <p>
	 * The broadcast can be executed in parallel, which can be enabled by setting
	 * {@link org.glassfish.tyrus.core.TyrusWebSocketEngine#PARALLEL_BROADCAST_ENABLED}
	 ...
	 * A negative value or 0 means that sending of periodic Pong messages is not
	 * turned on.
	 *
	 * @return heartbeatInterval interval between periodic pong messages in
	 *         milliseconds.
	 */
	 ...
	 * Setting the interval to a negative value or 0 will cancel sending of periodic
	 * Pong messages.
	 *
	 * @param heartbeatInterval interval between periodic Pong messages in
	 *                          milliseconds.
	 */
	...
	/**
	 * Get the Internet Protocol (IP) address of the client or last proxy that sent
	 * the request.
	 *
	 * @return a {@link String} containing the IP address of the client that sent
	 *         the request or {@code null} when method is called on client-side.
	 */
	...
	enum State {
		...
		/**
		 * {@link Session} is running and is not receiving partial messages on
		 * registered {@link MessageHandler.Whole}.
		 */
		RUNNING,

		/**
		 * {@link Session} is currently receiving text partial message on registered
		 * {@link MessageHandler.Whole}.
		 */
		RECEIVING_TEXT,

		/**
		 * {@link Session} is currently receiving binary partial message on registered
		 * {@link MessageHandler.Whole}.
		 */
		RECEIVING_BINARY,
	...
	private class IdleTimeoutCommand implements Runnable {
		...
		public void run() {
			...
			// condition is required because scheduled task can be (for some reason) run
			// even when it is cancelled.
