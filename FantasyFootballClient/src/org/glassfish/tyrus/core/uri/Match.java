 ...
 * If the match is not exact, it means that some of the path segments on the
 * endpoint are variables. In this case, the Match object carries the indices of
 * the variable segments in the endpoint path, the map of the parameter names
 * and values.
 *
 * @author dannycoward
 */
public class Match {
	...
	/**
	 * {@code true} if the path of the matched endpoint does not contain any
	 * variables, {@code false} otherwise.
	 *
	 * @return {@code true} if the path of the matched endpoint does not contain any
	 *         variables, {@code false} otherwise.
	 */
	boolean isExact() {
	...
	/**
	 * Return a list of all endpoints with path matching the request path. The
	 * endpoints are in order of match preference, best match first.
	 *
	...
	private static Match matchPath(String requestPath, TyrusEndpointWrapper endpoint, DebugContext debugContext) {
		...
		if (requestPathSegments.size() != endpointPathSegments.size()) {
			debugContext.appendTraceMessage(LOGGER, Level.FINE, DebugContext.Type.MESSAGE_IN, "URIs ", requestPath, " and ",
	 ...
	 * @return {@code true} when provided path are equivalent, {@code false}
	 *         otherwise.
	 */
