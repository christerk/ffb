 ...
 * <p>
 * The path pattern is post-fixed with a right hand pattern that consists of
 * either a matching group that matches zero or more path segments, see
 * {@link RightHandPath#capturingZeroOrMoreSegments}, or zero path segments, see
 * {@link RightHandPath#capturingZeroSegments}.
 *
 * @author Paul Sandoz
 */
public final class PathPattern extends PatternWithGroups {
	...
	/**
	 * Path pattern comparator that defers to {@link UriTemplate#COMPARATOR
	 * comparing the templates} associated with the patterns.
	 */
	...
	public static enum RightHandPath {
		...
		/**
		 * A capturing group that matches zero or more path segments and keeps the
		 * matching path template open.
		 */
		capturingZeroOrMoreSegments("(/.*)?"),
		/**
		 * A capturing group that matches zero segments and effectively closes the
		 * matching path template.
		 */
	 ...
	 * {@link RightHandPath#capturingZeroSegments closed} right hand path.
	 *
	...
	public PathPattern(PathTemplate template) {
		super(postfixWithCapturingGroup(template.getPattern().getRegex()), addIndexForRightHandPathCapturingGroup(
				template.getNumberOfRegexGroups(), template.getPattern().getGroupIndexes()));
	...
	public PathPattern(PathTemplate template, RightHandPath rhpp) {
		super(postfixWithCapturingGroup(template.getPattern().getRegex(), rhpp), addIndexForRightHandPathCapturingGroup(
				template.getNumberOfRegexGroups(), template.getPattern().getGroupIndexes()));
