...
public class RemoteSession implements Session, DistributedSession {
	...
	public static enum DistributedMapKey implements Serializable {
		 ...
		 * <p>
		 * Value must be {@link java.util.Map}&lt;{@link String},
		 * {@link java.util.List}&lt;{@link String}&gt;&gt;.
		 *
	 ...
	 */
	public RemoteSession(final String sessionId, final ClusterContext clusterContext,
			final Map<DistributedMapKey, Object> distributedPropertyMap, final TyrusEndpointWrapper endpointWrapper,
		...
		this.basicRemote = new RemoteEndpoint.Basic() {
			 ...
			 * <p>
			 * {@link java.util.concurrent.Future#get()} will be invoked and exception
			 * processed (if thrown).
			 *
			 * @param future to be processed.
			 * @throws IOException when {@link java.io.IOException} is the cause of thrown
			 *                     {@link java.util .concurrent.ExecutionException} it will
			 *                     be extracted and rethrown. Otherwise whole
			 *                     ExecutionException will be rethrown wrapped in
			...
			public void sendObject(Object data) throws IOException, EncodeException {
				...
				// TODO 2: uncertain what happens when current session is closed anytime during
				// the process
	...
	/**
	 * Get the sub protocol agreed during the websocket handshake for this
	 * conversation.
	 *
	 ...
	 *
	 * @return {@code true} when the underlying socket is using a secure transport,
	 ...
	 *
	 * @return {@code true} when the underlying socket is open, {@code false}
	 *         otherwise.
	 */
	@Override
	 ...
	 * container if it is inactive, i.e. no messages are either sent or received in
	 * that time.
	 *
	...
	/**
	 * Get the maximum length of incoming binary messages that this Session can
	 * buffer. If the implementation receives a binary message that it cannot buffer
	 * because it is too large, it must close the session with a close code of
	...
	/**
	 * Get the maximum length of incoming text messages that this Session can
	 * buffer. If the implementation receives a text message that it cannot buffer
	 * because it is too large, it must close the session with a close code of
	 * {@link CloseReason.CloseCodes#TOO_BIG}.
	 *
	...
	/**
	 * Get a reference a {@link RemoteEndpoint.Async} object representing the peer
	 * of this conversation that is able to send messages asynchronously to the
	 * peer.
	 *
	...
	/**
	 * Get a reference a {@link RemoteEndpoint.Basic} object representing the peer
	...
	/**
	 * Get a string containing the unique identifier assigned to this session. The
	 * identifier is assigned by the web socket implementation and is implementation
	 * dependent.
	 *
	...
	/**
	 * Get the {@link URI} under which this session was opened, including the query
	...
	/**
	 * Get the request parameters associated with the request this session was
	 * opened under.
	 *
	...
	/**
	 * Get the query string associated with the request this session was opened
	 * under.
	 *
	...
	/**
	 * This method is not supported on {@link RemoteSession}. Each invocation will
	...
	/**
	 * Get the authenticated user for this session or {@code null} if no user is
