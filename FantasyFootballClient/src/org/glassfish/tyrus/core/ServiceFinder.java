 ...
 * <p>
 * The default service provider registration/lookup mechanism based on
 * <tt>META-INF/services</tt> files is described below. For environments, where
 * the basic mechanism is not suitable, clients can enforce a different approach
 * by setting their custom <tt>ServiceIteratorProvider</tt> by calling
 * <tt>setIteratorProvider</tt>. The call must be made prior to any lookup
 * attempts.
 * <p>
 * A service provider identifies itself by placing a provider-configuration file
 * in the resource directory <tt>META-INF/services</tt>. The file's name should
 ...
 * <tt>META-INF/services/java.io.spi.CharCodec</tt>. This file would contain the
 * single line:
 * 
 * <pre>
 ...
 * </pre>
 * <p>
 * To locate an codec for a given encoding name, the internal I/O code would do
 * something like this:
 * 
 * <pre>
...
public final class ServiceFinder<T> implements Iterable<T> {
	 ...
	 * <tt>getResources</tt> method of the given class loader to find all available
	 * files with that name. These files are then read and parsed to produce a list
	 * of provider-class names. The iterator that is returned uses the given class
	 ...
	 * @throws ServiceConfigurationError If a provider-configuration file violates
	 *                                   the specified format or names a provider
	 ...
	 *                              provider-configuration files and instantiate
	 *                              provider classes, or <tt>null</tt> if the system
	 *                              class loader (or, failing that the bootstrap
	 *                              class loader) is to be used
	 * @param ignoreOnClassNotFound If a provider cannot be loaded by the class
	 *                              loader then move on to the next available
	 *                              provider.
	 * @param <T>                   the type of the service instance.
	 * @return the service finder
	 * @throws ServiceConfigurationError If a provider-configuration file violates
	...
	/**
	 * Locates and incrementally instantiates the available providers of a given
	 * service using the context class loader. This convenience method is equivalent
	 * to
	 * 
	 * <pre>
	 ...
	 *
	 * @param serviceName the service name correspond to a file in META-INF/services
	 *                    that contains a list of fully qualified class names
	 * @return the service finder
	 * @throws ServiceConfigurationError If a service file violates the specified
	 *                                   format or names a provider class that
	...
	/**
	 * Register the service iterator provider to iterate on provider instances or
	 * classes.
	 * <p>
	 * The default implementation registered,
	 * {@link DefaultServiceIteratorProvider}, looks up provider classes in
	 * META-INF/service files.
	 * <p>
	 * This method must be called prior to any attempts to obtain provider instances
	 * or classes.
	 *
	 ...
	 *         <tt>ServiceConfigurationError</tt> if a provider-configuration file
	 *         violates the specified format or if a provider class cannot be found
	 *         and instantiated.
	 */
	@Override
	...
	public Class<T>[] toClassArray() throws ServiceConfigurationError {
		...
		Iterator<Class<T>> i = iteratorProvider.createClassIterator(serviceClass, serviceName, classLoader,
				ignoreOnClassNotFound);
		while (i.hasNext()) {
			result.add(i.next());
		}
	...
	/**
	 * Parse a single line from the given configuration file, adding the name on the
	 * line to both the names list and the returned set iff the name is not already
	 * a member of the returned set.
	 */
	private static int parseLine(String serviceName, URL u, BufferedReader r, int lc, List<String> names,
	 ...
	 *                    already been returned. This set will be updated to contain
	 *                    the names that will be yielded from the returned
	 *                    <tt>Iterator</tt>.
	 * @return A (possibly empty) <tt>Iterator</tt> that will yield the
	 *         provider-class names in the given configuration file that are not yet
	 *         members of the returned set
	 * @throws ServiceConfigurationError If an I/O error occurs while reading from
	 *                                   the given URL, or if a configuration-file
	...
	private static class AbstractLazyIterator<T> {
		...
		private AbstractLazyIterator(Class<T> service, String serviceName, ClassLoader loader,
					...
					try {
						...
						if (thrown instanceof ClassNotFoundException) {
							...
							if (LOGGER.isLoggable(Level.CONFIG)) {
								...
								LOGGER.log(Level.CONFIG, LocalizationMessages
										.DEPENDENT_CLASS_OF_PROVIDER_NOT_FOUND(thrown.getLocalizedMessage(), nextName, service));
							}
							nextName = null;
							...
							if (LOGGER.isLoggable(Level.CONFIG)) {
								LOGGER.log(Level.CONFIG, LocalizationMessages
										.DEPENDENT_CLASS_OF_PROVIDER_FORMAT_ERROR(thrown.getLocalizedMessage(), nextName, service));
							}
							nextName = null;
		...
		private void handleClassNotFoundException() {
			...
			if (LOGGER.isLoggable(Level.CONFIG)) {
				LOGGER.log(Level.CONFIG, LocalizationMessages.PROVIDER_NOT_FOUND(nextName, service));
			}
			nextName = null;
		}
	}
	...
	private static final class LazyClassIterator<T> extends AbstractLazyIterator<T> implements Iterator<Class<T>> {
		...
		public Class<T> next() {
			...
			try {
				...
				fail(serviceName, LocalizationMessages.PROVIDER_NOT_FOUND(cn, service));
			} catch (PrivilegedActionException pae) {
				...
				if (thrown instanceof ClassNotFoundException) {
					...
					fail(serviceName,
							LocalizationMessages.DEPENDENT_CLASS_OF_PROVIDER_NOT_FOUND(thrown.getLocalizedMessage(), cn, service));
				} else if (thrown instanceof ClassFormatError) {
					fail(serviceName,
							LocalizationMessages.DEPENDENT_CLASS_OF_PROVIDER_FORMAT_ERROR(thrown.getLocalizedMessage(), cn, service));
				} else {
					fail(serviceName,
	...
	private static final class LazyObjectIterator<T> extends AbstractLazyIterator<T> implements Iterator<T> {
		...
		private LazyObjectIterator(Class<T> service, String serviceName, ClassLoader loader,
				boolean ignoreOnClassNotFound) {
			super(service, serviceName, loader, ignoreOnClassNotFound);
		}
			...
			while (nextName == null) {
				...
				try {
					t = service.cast(AccessController
							.doPrivileged(ReflectionHelper.classForNameWithExceptionPEA(nextName, loader)).newInstance());
					...
					if (ignoreOnClassNotFound) {
						...
						fail(serviceName,
								LocalizationMessages.PROVIDER_COULD_NOT_BE_CREATED(nextName, service, ex.getLocalizedMessage()), ex);
					}
					...
					if (cause instanceof ClassNotFoundException) {
						...
						if (ignoreOnClassNotFound) {
							if (LOGGER.isLoggable(Level.CONFIG)) {
								LOGGER.log(Level.CONFIG, LocalizationMessages
										.DEPENDENT_CLASS_OF_PROVIDER_FORMAT_ERROR(cause.getLocalizedMessage(), nextName, service));
							}
							nextName = null;
						} else {
							fail(serviceName, LocalizationMessages
									.DEPENDENT_CLASS_OF_PROVIDER_FORMAT_ERROR(cause.getLocalizedMessage(), nextName, service), cause);
						}
					} else {
						fail(serviceName,
		...
		private void handleClassNotFoundException() throws ServiceConfigurationError {
			if (ignoreOnClassNotFound) {
				...
				fail(serviceName, LocalizationMessages.PROVIDER_NOT_FOUND(nextName, service));
			}
		}
	}
	 ...
	 * files, see {@link DefaultServiceIteratorProvider}. This implementation may be
	 * overridden by invoking
	...
	public abstract static class ServiceIteratorProvider {
		...
		private static ServiceIteratorProvider getInstance() {
			// TODO: check the following is a good practice: Double-check idiom for lazy
		 ...
		 * @param loader                the class loader to utilize when loading
		 *                              provider classes.
		 * @param ignoreOnClassNotFound if true ignore an instance if the corresponding
		 *                              provider class if cannot be found, otherwise
		 *                              throw a {@link ClassNotFoundException}.
		 * @return the provider instance iterator.
		 */
		public abstract <T> Iterator<T> createIterator(Class<T> service, String serviceName, ClassLoader loader,
				boolean ignoreOnClassNotFound);
		 ...
		 * @param loader                the class loader to utilize when loading
		 *                              provider classes.
		 * @param ignoreOnClassNotFound if true ignore the provider class if cannot be
		 *                              found, otherwise throw a
		 *                              {@link ClassNotFoundException}.
		 * @return the provider class iterator.
		 */
		public abstract <T> Iterator<Class<T>> createClassIterator(Class<T> service, String serviceName, ClassLoader loader,
				boolean ignoreOnClassNotFound);
	}
	 ...
	 * <p>
	 * This class may utilized if a {@link ServiceIteratorProvider} needs to reuse
