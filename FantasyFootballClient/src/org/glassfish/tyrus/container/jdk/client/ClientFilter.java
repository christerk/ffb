...
class ClientFilter extends Filter {
	 ...
	 * Constructor.
	 *
	 * @param downstreamFilter a filer that is positioned directly under this
	 *                         filter.
	 * @param clientEngine     client engine instance.
	 * @param properties       client properties.
	 * @param jdkConnector     callback to connecting with modified
	 *                         {@link UpgradeRequest} if necessary.
	 * @param upgradeRequest   upgrade request to be used for this client session.
	 */
	// * @param proxyHeaders map representing headers to be added to request sent to
	// proxy (HTTP CONNECT).
	ClientFilter(Filter downstreamFilter, ClientEngine clientEngine, Map<String, Object> properties,
			Callable<Void> jdkConnector, UpgradeRequest upgradeRequest) throws DeploymentException {
	...
	/**
	 * @param address                  an address where to connect (server or
	 *                                 proxy).
	 * @param proxy                    {@code true} if the connection will be
	 *                                 established via proxy, {@code false}
	 *                                 otherwise.
	...
	public boolean processRead(ByteBuffer data) {
		if (wsConnection == null) {
			...
			ClientEngine.ClientUpgradeInfo clientUpgradeInfo = clientEngine.processResponse(tyrusUpgradeResponse, writer,
					new CloseListener() {
	...
	void processError(Throwable t) {
		// connectCompletionHandler != null means that we are still in "connecting
		// state".
	...
	private static Map<String, String> getProxyHeaders(Map<String, Object> properties) throws DeploymentException {
		...
		if (proxyHeaders == null) {
			if (wlsProxyUsername != null && wlsProxyPassword != null) {
				...
				proxyHeaders.put("Proxy-Authorization", "Basic " + Base64Utils
						.encodeToString((wlsProxyUsername + ":" + wlsProxyPassword).getBytes(Charset.forName("UTF-8")), false));
			}
		} else {
			...
			// if (proxyAuthPresent == true) then do nothing, proxy authorization header is
			// already added.
