...
public class JdkClientContainer implements ClientContainer {
	 ...
	 * {@link org.glassfish.tyrus.container.jdk.client.TransportFilter} when SSL is
	 * turned on. The size cannot be smaller than a maximal size of a SSL packet,
	 * which is 16kB for payload + header, because
	 * {@link org.glassfish.tyrus.container.jdk.client.SslFilter} does not have its
	 * own buffer for buffering incoming
	 * {@link org.glassfish.tyrus.container.jdk.client.SslFilter} does not have its
	 * own buffer for buffering incoming data and therefore the entire SSL packet
	 * must fit into
	 * {@link org.glassfish.tyrus.container.jdk.client.TransportFilter} input
	 * buffer.
	 */
	...
	public void openClientSocket(final ClientEndpointConfig cec, final Map<String, Object> properties,
		...
		ThreadPoolConfig threadPoolConfig = Utils.getProperty(properties, ClientProperties.WORKER_THREAD_POOL_CONFIG,
				ThreadPoolConfig.class);
		if (threadPoolConfig == null) {
			threadPoolConfig = ThreadPoolConfig.defaultConfig();
		}
		// weblogic.websocket.client.max-aio-threads has priority over what is in thread
		// pool config
		...
		final Integer containerIdleTimeout = Utils.getProperty(properties, ClientProperties.SHARED_CONTAINER_IDLE_TIMEOUT,
				Integer.class);
		...
		final Callable<Void> jdkConnector = new Callable<Void>() {
			...
			public Void call() throws DeploymentException {
				...
				if (secure) {
					TransportFilter transportFilter = createTransportFilter(SSL_INPUT_BUFFER_SIZE, finalThreadPoolConfig,
					...
					TransportFilter transportFilter = createTransportFilter(INPUT_BUFFER_SIZE, finalThreadPoolConfig,
							containerIdleTimeout);
					writeQueue = createTaskQueueFilter(transportFilter);
				}

				final ClientFilter clientFilter = createClientFilter(properties, writeQueue, clientEngine, this,
						upgradeRequest);
	...
	private SslFilter createSslFilter(ClientEndpointConfig cec, Map<String, Object> properties,
		...
		if (sslEngineConfiguratorObject != null) {
			// property is set, we need to figure out whether new or deprecated one is used
			// and act accordingly.
			if (sslEngineConfiguratorObject instanceof SslEngineConfigurator) {
				...
				sslFilter = new SslFilter(transportFilter,
						(org.glassfish.tyrus.container.jdk.client.SslEngineConfigurator) sslEngineConfiguratorObject);
			} else {
		...
		// if we are trying to access "wss" scheme and we don't have
		...
		if (sslFilter == null) {
			...
			String wlsSslTrustStorePassword = (String) cec.getUserProperties()
					.get(ClientManager.WLS_SSL_TRUSTSTORE_PWD_PROPERTY);
			...
			String wlsIgnoreHostnameVerification = System.getProperties()
					.getProperty(ClientManager.WLS_IGNORE_HOSTNAME_VERIFICATION);
			if ("true".equalsIgnoreCase(wlsIgnoreHostnameVerification)) {
				sslEngineConfigurator.setHostVerificationEnabled(false);
			} else {

				// if hostname verification is not ignored, Tyrus looks for {@value
				// ClientManager
				// .WLS_HOSTNAME_VERIFIER_CLASS}.
				// If that is found and the class can be instantiated properly, it will be used
				// as Hostname
	...
	private ClientFilter createClientFilter(Map<String, Object> properties, Filter downstreamFilter,
			ClientEngine clientEngine, Callable<Void> jdkConnector, UpgradeRequest upgradeRequest)
	 ...
	 */
	private void connectSynchronously(ClientFilter clientFilter, final SocketAddress address, boolean proxy)
			throws Throwable {
		...
		try {
			...
			throw new DeploymentException(
					"The thread waiting for client to connect has been interrupted before the connection has finished", e);
		}
	}
	...
	private List<Proxy> processProxy(Map<String, Object> properties, URI uri) throws DeploymentException {
		...
		if (wlsProxyHost != null) {
			proxies.add(
					new Proxy(Proxy.Type.HTTP, new InetSocketAddress(wlsProxyHost, wlsProxyPort == null ? 80 : wlsProxyPort)));
		} else {
		...
		if (proxySelector != null) {
			// see WebSocket Protocol RFC, chapter 4.1.3:
	...
	private void addProxies(ProxySelector proxySelector, URI uri, String scheme, List<Proxy> proxies) {
		for (Proxy p : proxySelector.select(getProxyUri(uri, scheme))) {
			...
			case HTTP:
				...
				break;
			case SOCKS:
				LOGGER.log(Level.INFO, String.format("Socks proxy is not supported, please file new issue at https://java"
	...
	private URI getProxyUri(URI wsUri, String scheme) {
		try {
			return new URI(scheme, wsUri.getUserInfo(), wsUri.getHost(), wsUri.getPort(), wsUri.getPath(), wsUri.getQuery(),
					wsUri.getFragment());
		} catch (URISyntaxException e) {
