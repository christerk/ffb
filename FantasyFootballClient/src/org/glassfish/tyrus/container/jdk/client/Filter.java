...
/**
 * A filter can add functionality to JDK client transport. Filters are composed
 * together to create JDK client transport.
 *
...
class Filter {
	 ...
	 * Constructor.
	 *
	 * @param downstreamFilter downstream filter. Accessible directly as
	 *                         {@link #downstreamFilter} protected field.
	 */
	...
	/**
	 * Perform write operation for this filter and invokes write method on the next
	 * filter in the filter chain.
	 *
	 * @param data              on which write operation is performed.
	 * @param completionHandler will be invoked when the write operation is
	...
	/**
	 * Close the filter, invokes close operation on the next filter in the filter
	 * chain.
	 * <p/>
	 * The filter is expected to clean up any allocated resources and pass the
	 * invocation to downstream filter.
	 */
	void close() {
	}

	/**
	 * Signal to turn on SSL, it is passed on in the filter chain until a filter
	 * responsible for SSL is reached.
	 */
	void startSsl() {
	}
	 ...
	 * Initiate connect.
	 * <p/>
	 * If the {@link Filter} needs to do something during this phase, it must
	 * implement {@link #handleConnect(SocketAddress, Filter)} method.
	 *
	...
	/**
	 * An event listener that is called when a connection is set up. This event
	 * travels up in the filter chain.
	 * <p/>
	 * If the {@link Filter} needs to process this event, it must implement
	 * {@link #processConnect()} method.
	 */
	 ...
	 * <p/>
	 * If the {@link Filter} needs to process this event, it must implement
	 * {@link #processRead(ByteBuffer)} ()} method. If the method returns
	 * {@code true}, the processing will continue with upstream filters; if the
	 * method invocation returns {@code false}, the processing won't continue.
	 *
	 ...
	 * <p/>
	 * If the {@link Filter} needs to process this event, it must implement
	 * {@link #processConnectionClosed()} method.
	 */
	 ...
	 * <p/>
	 * If the {@link Filter} needs to process this event, it must implement
	 * {@link #processSslHandshakeCompleted()} method.
	 */
	 ...
	 * <p/>
	 * If the {@link Filter} needs to process this event, it must implement
	 * {@link #processError(Throwable)} method.
	 *
	 ...
	 * @return {@code true} if the data should be sent to processing to upper filter
	 *         in the chain, {@code false} otherwise.
