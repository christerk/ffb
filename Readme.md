...
| Arguments                          | Description                                                                                                                                                                                                                                                                                                                    |
|------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [mode]                             | * -player - start client as player <br/> * -spectator - to join as specatator <br/> * -replay - to load replay data <br/> * -noCoachNoConnection - to load client in standalone mode without connection, it will allow yout to load jnlp file to initiate spectator or player mode whatever is configured in loaded jnlp file. |
...
| -jnlpOnInput [path-to-jnlp-file]   | This will load all arguments from passed `jnlp` file and initialize client with them overriding any previously passed arguments                                                                                                                                                                                                |

You also have ability to run client without arguments, that way it will start in `NO_COACH_NO_CONNECTION` mode and will allow you to load JNLP file using file chooser dialog to initialize client with argument from `jnlp` file.
